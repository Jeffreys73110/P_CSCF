...
CSIP_Server::CSIP_Server(CWnd* pParent /*=NULL*/)
	: CDialog(CSIP_Server::IDD, pParent)
{
...
int  CSIP_Server::Start()
{
	...
	// Initial Variable
    Write_Flag=FALSE; GetDlgItem(IDC_BUTTON1)->SetWindowText("Enable Log File");
	Relay_100_Flag=0; sprintf(S1,"Relay Level %d",Relay_100_Flag); GetDlgItem(IDC_BUTTON3)->SetWindowText(S1);
	RTP_Relay_IP[0]=RTP_Relay_Port[0]=0;
	Outbound_PhoneNo[0]=Log_File[0]=0;
	NTUT_GW_Socket=0; memset(&NTUT_GW_Addr,0,sizeof(NTUT_GW_Addr)); NTUT_GW_IP[0]=NTUT_GW_Port[0]=0;
	Access_CDMS_Flag=FALSE;

	// Set lpSIP_Event_Table
	memset(lpSIP_Event_Table,0,sizeof(lpSIP_Event_Table)); Get_Response_Code(NULL,0);
	...
	{
		if ((SIP_Server_Port[j]=Port[j])==0)	break;
		if (i=Start_UDP_Server(&SIP_Server_Socket[j],SIP_Server_Port[j],SIP_SERVER_EVENT,m_hWnd))
		{  sprintf(ErrMsg,"Start UDP(%d) Server Err",SIP_Server_Port[j]); }
		Server_Port_Num++;
	}
...
void CSIP_Server::OnSIP_Server(WPARAM wParam, LPARAM lParam)
{
	int         Len,i,ii;
	char        S1[SIP_MSG_LEGNTH+10];//,S2[2000],S3[2000];
	sockaddr_in Addr;
	...
	{  case FD_READ:      
	        // 1.讀取 SIP Message
			Len=sizeof(sockaddr);
	        ii=recvfrom(wParam,S1,sizeof(S1)-1,0,(sockaddr *)&Addr,&Len); if (ii<=0) break; else S1[ii]=0;

			// Check Cryption
			if		(check_sippacket(S1,ii)==1)	{ sip_decrypt(S1,ii,S1); S1[ii-4]=0; Crypto_Type=CT_SIPCrypto;}
			else if (walkersun_check_sippacket(S1,ii)==1)	{ walkersun_sip_decrypt(S1,ii,S1); Crypto_Type=CT_Walkersun1;}
			else	Crypto_Type=CT_NoCrypto;

			// 2.問題封包, 不處理
			if (ii<10) { sendto(wParam,S1,ii,0,(sockaddr *)&Addr,Len); break; }
			else if (ii<100)	break;
			else if (ii>SIP_MSG_LEGNTH)	break;
            
			// 3.分析 SIP Message
	        if ( (i=SIP_Message_Analyzer(wParam,&Addr,S1,&SIP_Msg,Crypto_Type))==0)
            {  
				Write_Log("recvfrom",S1,&Addr,&SIP_Msg);
				if        (!strcmp(SIP_Msg.Command,"REGISTER"))                    Process_REGISTER(&SIP_Msg);
				else if   (!strcmp(SIP_Msg.Command,"ACK"))                         Process_ACK(&SIP_Msg);
				else if   (!strcmp(SIP_Msg.Command,"INVITE"))                      Process_Proxy_Auth(&SIP_Msg);
				else if   (!strcmp(SIP_Msg.Command,"CANCEL"))                      Process_CANCEL(&SIP_Msg);
				else if   (!strcmp(SIP_Msg.Command,"PRACK"))					   Process_PRACK(&SIP_Msg);
				else if   (!strcmp(SIP_Msg.Command,"BYE"))                         Process_BYE(&SIP_Msg);
				else if   (!strcmp(SIP_Msg.Command,"REFER"))                       Process_REFER(&SIP_Msg);
				else if   (!strcmp(SIP_Msg.Command,"NOTIFY"))                      Process_NOTIFY(&SIP_Msg);
				else if ( (SIP_Msg.Command[0]>='0') && (SIP_Msg.Command[0]<='9') ) Process_STATUS(&SIP_Msg);
				else	{ if (ii<10)	{ ii=sendto(wParam,S1,ii,0,(sockaddr *)&Addr,Len);}}
			}
...
void CSIP_Server::Process_CANCEL(SIP_MSG *lpSIP_Msg)
{
    ...
    // 1. Duplicated "CANCEL"
	if ( (Cur=Check_Command(lpSIP_Msg)) >=0 ) Send_SIP_Command(lpSIP_Msg->Command,lpSIP_Msg,ADD_VIA,TO);

	// 2. CANCEL Request: Forwarding
	else if ( (lpSIP_Msg->TSocket!=NULL) && ((Cur=Save_Command(lpSIP_Msg))>=0) )
	{  // Check if INVITE Process Exist
	   ...
	   Command_Data[Cur].State=SIP_CANCEL; Command_Data[Cur].Timer_Counter=0;	
	}

	// 3. To: OffLine or Command Buffer Full
	else { Send_SIP_Command("503",lpSIP_Msg,NOCHANGE_VIA,FROM);} // Via OK
}
...
void CSIP_Server::Process_BYE(SIP_MSG *lpSIP_Msg)
{
	...
	{  // Check if INVITE Process Exist
	   memcpy(&SIP_Data,lpSIP_Msg,sizeof(SIP_MSG)); strcpy(SIP_Data.Command,"INVITE");
	   if ( (Cur111=Check_Command(&SIP_Data)) >= 0 ) {  if (Command_Data[Cur111].State!=SIP_READY) Clear_Command(Cur111); }
	   else
	   {  strcpy(S1,SIP_Data.From_Name); strcpy(SIP_Data.From_Name,SIP_Data.To_Name); strcpy(SIP_Data.To_Name,S1);
	      if ( (Cur111=Check_Command(&SIP_Data)) >= 0 ) { if (Command_Data[Cur111].State!=SIP_READY) Clear_Command(Cur111); }  }
	   
       Send_SIP_Command(lpSIP_Msg->Command,lpSIP_Msg,ADD_VIA,TO);   
	   SetTimer(TIMERID_COMMAND+Cur,TWO_SECOND,NULL);
	   Command_Data[Cur].State=SIP_BYE; Command_Data[Cur].Timer_Counter=0;
	   Invite_Proc(FROM, lpSIP_Msg, SIP_READY);
	}
...
void CSIP_Server::Process_REFER(SIP_MSG *lpSIP_Msg)
{
	...
	{  // Check if INVITE Process Exist
       Send_SIP_Command(lpSIP_Msg->Command,lpSIP_Msg,ADD_VIA,TO);   
	   SetTimer(TIMERID_COMMAND+Cur,TWO_SECOND,NULL);
	   Command_Data[Cur].State=SIP_REFER; Command_Data[Cur].Timer_Counter=0;	}
...
void CSIP_Server::Process_NOTIFY(SIP_MSG *lpSIP_Msg)
{
	...
	{  // Check if INVITE Process Exist
       Send_SIP_Command(lpSIP_Msg->Command,lpSIP_Msg,ADD_VIA,TO);   
	   SetTimer(TIMERID_COMMAND+Cur,TWO_SECOND,NULL);
	   Command_Data[Cur].State=SIP_NOTIFY; Command_Data[Cur].Timer_Counter=0;	}
...
void CSIP_Server::Process_REGISTER(SIP_MSG *lpSIP_Msg)
{
	...
	if		(Reg_Auth_Level==4)	{ Reg_Auth=TRUE;	Reg_VAddr_Check=FALSE;	Reg_CNonce_Check=TRUE;	}
	else if (Reg_Auth_Level==3)	{ Reg_Auth=TRUE;	Reg_VAddr_Check=FALSE;	Reg_CNonce_Check=FALSE;	}
	else if (Reg_Auth_Level==2)	{ Reg_Auth=TRUE;	Reg_VAddr_Check=TRUE;	Reg_CNonce_Check=TRUE;	}
	else if (Reg_Auth_Level==1)	{ Reg_Auth=TRUE;	Reg_VAddr_Check=TRUE;	Reg_CNonce_Check=FALSE;	}
	else if (Reg_Auth_Level==0)	{ Reg_Auth=FALSE;	Reg_VAddr_Check=FALSE;	Reg_CNonce_Check=FALSE;	}
	
	if ((Online_Cur=Check_OnLine(lpSIP_Msg->From_Name))>=0)
	{	First_Flag = FALSE;
		Vary_Flag = (Addrcmp(&lpSIP_Msg->FAddr,&SIP_OnLine_Data[Online_Cur].Addr))? TRUE:FALSE;
	...
	{	Vary_Flag = TRUE;
		...
		{	// Server's Resource Fail
			if (Online_Cur==-1) Send_Auth_Msg("503",lpSIP_Msg,NOCHANGE_VIA,FROM); // Via OK
			// Account don't conform norms
			else if (Online_Cur<0) Send_Auth_Msg("403",lpSIP_Msg,NOCHANGE_VIA,FROM); // Via OK
			return;
	...
	{	Check_MD5(lpSIP_Msg,Online_Cur,TRUE);//Access_CDMS_Flag
		// 帳號錯誤, 啥事都不做
		if (SIP_OnLine_Data[Online_Cur].CDMS_OK!=1)	{ Clear_OnLine(lpSIP_Msg); return;}
		if (Reg_Auth_Level==0)	SIP_OnLine_Data[Online_Cur].MD5_OK = TRUE;
		if (Proxy_Auth_Level==0||(Proxy_Auth_Level==1 && SIP_OnLine_Data[Online_Cur].MD5_OK))	SIP_OnLine_Data[Online_Cur].Proxy_MD5_OK = TRUE;
	...
	if ((Command_Cur=Check_Command(lpSIP_Msg))>=0 && Command_Data[Command_Cur].State==SIP_REG)
	{
		if (!SIP_OnLine_Data[Online_Cur].MD5_OK)
		{
			...
			{	SetTimer(TIMERID_COMMAND+Command_Cur,TWO_SECOND,NULL);
				if (Reg_ANonce_Flag)	Proc_Auth(lpSIP_Msg,Command_Cur,Online_Cur,"401");	// Reassign new Nonce
		...
		{	// Modify Expires
			if (!First_Flag && Modify_Expire_Check)
			{
				...
				{	if (atoi(lpSIP_Msg->Expires)!=0)
					{	if (Vary_Flag)	Step=(SIP_OnLine_Data[Online_Cur].Expires_OK)? atoi(lpSIP_Msg->Expires)-5 : atoi(lpSIP_Msg->Expires)/2;
						else Step=atoi(lpSIP_Msg->Expires)+5; SIP_OnLine_Data[Online_Cur].Expires_OK=TRUE;
					}else Step=0;
					Modify_Expires(lpSIP_Msg,Step);
				}else if (Locking_Expires>0)	Modify_Expires(lpSIP_Msg,Locking_Expires);				
			}
			Save_OnLine(lpSIP_Msg,FROM);	//Already Online
			KillTimer(TIMERID_COMMAND+Command_Cur);
	...
	else if ((Command_Cur=Save_Command(lpSIP_Msg))==-1)	{ Send_Auth_Msg("503",lpSIP_Msg,NOCHANGE_VIA,FROM); Clear_OnLine(lpSIP_Msg);}
...
void CSIP_Server::Process_Proxy_Auth(SIP_MSG *lpSIP_Msg)
{
	...
	// Set Param
	if		(Proxy_Auth_Level==3)	{ Proxy_Auth=TRUE;	Proxy_RMD5_Check=FALSE;	Proxy_CNonce_Check=TRUE;}
	else if (Proxy_Auth_Level==2)	{ Proxy_Auth=TRUE;	Proxy_RMD5_Check=FALSE; Proxy_CNonce_Check=FALSE;}
	else if (Proxy_Auth_Level==1)	{ Proxy_Auth=TRUE;	Proxy_RMD5_Check=TRUE;	Proxy_CNonce_Check=FALSE;}
	else if (Proxy_Auth_Level==0)	{ Proxy_Auth=FALSE;	Proxy_RMD5_Check=FALSE;	Proxy_CNonce_Check=FALSE;}
	...
	{	
		...
		if (Proxy_Auth_Level==0)	SIP_OnLine_Data[Online_Cur].Proxy_MD5_OK = TRUE;
	...
	{	// SIP_REG -> SIP_REG: REGISTER(Duplicate)
		...
		SetTimer(TIMERID_COMMAND+Command_Cur,TWO_SECOND,NULL);
		if (Proxy_ANonce_Flag)	Proc_Auth(lpSIP_Msg,Command_Cur,Online_Cur,"407");	// Reassign new Nonce
		Send_Auth_Msg(Command_Data[Command_Cur].Response_SIP_Msg.Command,&Command_Data[Command_Cur].Response_SIP_Msg,NOCHANGE_VIA,FROM);
		TRACE("Command_Cur>=0, =%d\r\n",Command_Cur);
	}
	...
	{	Save_OnLine(lpSIP_Msg,FROM);	//Already Online
		...
		{ if ((k=Get_NextLine(&lpSIP_Msg->Cmd_Str[j]))>0)   { k+=j; Deletion(lpSIP_Msg->Cmd_Str,j,k-1);}}	// Remove Proxy-Authorization
	...
	// New Command but Command Buffer Full 
	else if ((Command_Cur=Save_Command(lpSIP_Msg))==-1)	Send_Auth_Msg("503",lpSIP_Msg,NOCHANGE_VIA,FROM);
	...
	{	// Restore Partial Dial-No. function for Auth.
		...
		Send_Auth_Msg(Command_Data[Command_Cur].Response_SIP_Msg.Command,&Command_Data[Command_Cur].Response_SIP_Msg,NOCHANGE_VIA,FROM);
		Command_Data[Command_Cur].State=SIP_PROXY_AUTH; Command_Data[Command_Cur].Timer_Counter=0;
		TRACE("Proxy_Authorization\r\n");
	}
}

int	CSIP_Server::Send_Auth_Msg(char *Command,SIP_MSG *lpSIP_Data,int Via_Flag,int Send_Flag)
{
	int		i,k;
	char	S1[SIP_MSG_LEGNTH],S2[300];
	bool	Quintum_Flag;
	...
	if (strstr(lpSIP_Data->User_Agent,"Quintum")!=NULL)	{ Quintum_Flag=TRUE; Send_SIP_Command("100",lpSIP_Data,NOCHANGE_VIA,FROM);}

	// WWW-Authenticate:
    if (!strcmp(Command,"401"))
	{	if ((i=Get_Position(S1,"WWW-Authenticate:"))>0) 
		{ k=Get_NextLine(&S1[i])+i; Deletion(S1,i,k-1);}
		else if ((i=Get_Position(S1,"Contact:"))<0)
		{  if ((i=Get_Position(S1,"Via:"))<0) { if ((i=Get_Position(S1,"Expires:"))<0) i=Get_Position(S1,"CSeq:");}}
		if (i>0) 
		{	
			if (Quintum_Flag)
...
SIP_MSG SIP_Data; 			
	...
	{  
		...
		{  
			if (Command_Data[Cur111].Disconnect_Flag==FALSE) Send_SIP_Command(lpSIP_Msg->Command,lpSIP_Msg,ADD_VIA,TO);
		    Command_Data[Cur111].State=SIP_ACK; Command_Data[Cur111].Timer_Counter=0;
		...
		{   KillTimer(TIMERID_COMMAND+Cur111);
...
TRACE("INVITE----------------------------\r\n");
    ...
    if ( lpSIP_Msg->TSocket == NULL )	Send_SIP_Command("404",lpSIP_Msg,NOCHANGE_VIA,FROM);
	...
	{  
		...
		{ Send_SIP_Command(Command_Data[Command_Cur].Response_SIP_Msg.Command,&Command_Data[Command_Cur].Response_SIP_Msg,DELETE_VIA,FROM); }//ADD_VIA,FROM); }
	...
	{  Send_SIP_Command("503",lpSIP_Msg,NOCHANGE_VIA,FROM);  } // Via OK

	// 4. New Command(RELAY): SIP_READY -> SIP_RELAY_CONNECT
	else if ( (lpSIP_Msg->RTP_Relay_Flag) &&
		      ((i=Start_TCP_Client(&Command_Data[Command_Cur].Relay_TCP_Socket,0,atoi(lpSIP_Msg->Relay_TCP_Port),lpSIP_Msg->Relay_TCP_IP,RTP_RELAY_EVENT,m_hWnd))!=-100) )
	{  SetTimer(TIMERID_COMMAND+Command_Cur,TWO_SECOND,NULL); 
	   Command_Data[Command_Cur].State=SIP_RELAY_CONNECT; Command_Data[Command_Cur].Timer_Counter=0;
	}

	// 5. New Command(!Relay|Relay Fail): SIP_READY -> SIP_INVITE
	else 
	{  if (lpSIP_Msg->RTP_Relay_Flag) { lpSIP_Msg->RTP_Relay_Flag=FALSE; Command_Data[Command_Cur].SIP_Msg.RTP_Relay_Flag=FALSE; } // 強迫取消 Relay
	   Send_SIP_Command(lpSIP_Msg->Command,lpSIP_Msg,ADD_VIA,TO);
	   SetTimer(TIMERID_COMMAND+Command_Cur,TWO_SECOND,NULL);  
	   // Command_Data[Command_Cur].Timer_Counter=0; has been set at Item 5(Save_Command)
	   Command_Data[Command_Cur].State=SIP_INVITE; Command_Data[Command_Cur].Timer_Counter=0;
	}
}
...
void CSIP_Server::Process_STATUS(SIP_MSG *lpSIP_Msg)
{
	...
	{  
		...
		{   Modify_SDP(lpSIP_Msg,Command_Data[Cur111].Relay_UDP_IP,Command_Data[Cur111].Relay_UDP_FPort,Command_Data[Cur111].Relay_UDP_TPort); 
			if (Command_Data[Cur111].SIP_Msg.RL1_Flag==TRUE) {  Send_Relay_Command("0RL1",lpSIP_Msg,&Command_Data[Cur111]); Command_Data[Cur111].SIP_Msg.RL1_Flag=FALSE; }   
		}
		...
		{	
			...
			if (lpSIP_Msg->Command[0]=='1')	{ Command_Data[Cur111].State=SIP_100;  Command_Data[Cur111].Timer_Counter=0;}
			else if ( (lpSIP_Msg->Command[0]=='2') || (lpSIP_Msg->Command[0]=='3') || (lpSIP_Msg->Command[0]=='4') ) 
			{  
				Command_Data[Cur111].State=SIP_200;  Command_Data[Cur111].Timer_Counter=0;
		...
		else if (Command_Data[Cur111].State==SIP_ABANDON)
		{
			...
			{	
				...
				{	sprintf(lpSIP_Msg->Contact_Name,"002886%s",&lpSIP_Msg->To_Name[1]);
					...
					if ((Online_Cur=Check_OnLine(lpSIP_Msg->From_Name,&lpSIP_Msg->FAddr))>=0)	sprintf(lpSIP_Msg->Contact_Port,"%d",SIP_OnLine_Data[Online_Cur].Server_Port);
		...
		{  
			if ( (lpSIP_Msg->Command[0]=='2') || lpSIP_Msg->Command[0]=='3' || (lpSIP_Msg->Command[0]=='4') )
			{
				if (Command_Data[Cur111].Abandon_Flag && !strcmp(lpSIP_Msg->Command,"487"))	Send_SIP_Command("486",lpSIP_Msg,DELETE_VIA,FROM); // Via OK    
				else Send_SIP_Command(lpSIP_Msg->Command,lpSIP_Msg,DELETE_VIA,FROM); // Via OK
			}
		}
		...
		{	KillTimer(TIMERID_COMMAND+Cur111);
			Send_SIP_Command(lpSIP_Msg->Command,lpSIP_Msg,DELETE_VIA,FROM); // Via OK 
			if (Command_Data[Cur111].State==SIP_BYE || Command_Data[Cur111].State==SIP_CANCEL)	Invite_Proc(FROM, lpSIP_Msg, Command_Data[Cur111].State);
			Clear_Command(Cur111);
		}
		else if (Command_Data[Cur111].State==SIP_ACK)
		{  
...
void CSIP_Server::OnTimer(UINT nIDEvent) 
{
	...
	{	for (i=0,Cur=0;Cur<SIP_ONLINE_SIZE;Cur++)
		...
		{	if (SIP_OnLine_Num==0) { Cur=SIP_ONLINE_SIZE; break; } 
			...
			else	SIP_OnLine_Data[Cur].Expires_Counter++;
			// 定時向 Client 發 Packet, 以保持 NAT Channel Activity
			if ((SIP_OnLine_Data[Cur].Expires_Counter % UDP_ALIVE_TIME)==0)
			{  sendto(SIP_OnLine_Data[Cur].Socket,"test",4,0,(sockaddr *)&SIP_OnLine_Data[Cur].Addr,sizeof(sockaddr)); }
			i++; if (i>=SIP_OnLine_Num) { Cur=SIP_ONLINE_SIZE; break; }
		}
		sprintf(S1,"UAC=%d Relay=%d Com.Queue=%d Invite=%d",SIP_OnLine_Num,0,Command_Queue_Num,SIP_Invite_Num); 
		SetWindowText(S1);
	}
	...
	{	Cur=nIDEvent-TIMERID_COMMAND;
		...
		{	Command_Data[Cur].Timer_Counter+=TWO_SECOND;
			...
			{	KillTimer(nIDEvent);
				...
				if (Command_Data[Cur].State==SIP_BYE || Command_Data[Cur].State==SIP_CANCEL) Invite_Proc(EXTRA, &Command_Data[Cur].SIP_Msg, Command_Data[Cur].State);
				Clear_Command(Cur);
			}	   
		}
			...
			{	
				...
				{	// SubServer Route 2nd or 3rd SubServer...
					...
					if (OutBound_SubServer_Filter(&Command_Data[Cur].SIP_Msg,&Connect_Data))
					{
						...
						SIP_Msg2.TSocket=Connect_Data.Socket; 
						memcpy(&SIP_Msg2.TAddr,&Connect_Data.Addr,sizeof(sockaddr_in));
						strcpy(SIP_Msg2.TV_IP,Connect_Data.IP);	   
						strcpy(SIP_Msg2.TR_IP,Connect_Data.IP);
				...
				{	// Request Timoout more than 5 seconds and Forwarding to GW
					...
					if ((Online_Cur=Check_OnLine(Command_Data[Cur].Response_SIP_Msg.From_Name,&Command_Data[Cur].Response_SIP_Msg.FAddr))>=0)	sprintf(Command_Data[Cur].Response_SIP_Msg.Contact_Port,"%d",SIP_OnLine_Data[Online_Cur].Server_Port);	
					...
					Command_Data[Cur].State=SIP_200; Command_Data[Cur].Timer_Counter=0;
					...
					if ((Online_Cur=Check_OnLine(Command_Data[Cur].Response_SIP_Msg.To_Name,&Command_Data[Cur].Response_SIP_Msg.TAddr))>=0)
		...
		{	Command_Data[Cur].Timer_Counter+=TWO_SECOND;
			...
			{  
				...
				if (Command_Data[Cur].Abandon_Flag==2)	Command_Data[Cur].Abandon_Flag=(Forwarding_Mode_Check)? 2:3; //當設為2表示程式Forwarding GW為內部走法,若為3表示Forwarding GW走302
				// Abandon Proc
				if (Command_Data[Cur].Abandon_Flag==1)	Invite_Proc(TO,&Command_Data[Cur].SIP_Msg,SIP_100,"CANCEL");	//當Invite_Proc(To,.)時,最後一個Command才需要加
				else if (Command_Data[Cur].Abandon_Flag==2)
				{	
					if ((Invite_Cur=Check_Invite(&Command_Data[Cur].SIP_Msg))>=0)	SIP_Invite_Data[Invite_Cur].Forwarding_GW_Flag=TRUE;
					if ((Online_Cur=Check_OnLine(Command_Data[Cur].SIP_Msg.From_Name))>=0)
					{	// Re-send Invite
						memcpy(&SIP_Msg2,&Command_Data[Cur].SIP_Msg,sizeof(SIP_MSG));
						SIP_Msg2.TSocket=SIP_OnLine_Data[Online_Cur].Trunk_GW.Socket;
						memcpy(&SIP_Msg2.TAddr,&SIP_OnLine_Data[Online_Cur].Trunk_GW.Addr,sizeof(sockaddr_in));
						PostMessage(INVITE_EVENT,(WPARAM)&SIP_Msg2,(LPARAM)NULL);
					}
				}
				Command_Data[Cur].State=SIP_ABANDON; Command_Data[Cur].Timer_Counter=0;
			}
		}
		...
		{	Command_Data[Cur].Timer_Counter+=TWO_SECOND;
			...
			{  	
				...
				{	Modify_CSeq(Command_Data[Cur].SIP_Msg.Cmd_Str,"ACK");
					...
					Command_Data[Cur].State=SIP_ACK; Command_Data[Cur].Timer_Counter=0;
					//在Abandon_Flag=2時不帶入Invite_Proc(.),因此通電話位結束，將導到GW
				}
		...
		{	Command_Data[Cur].Timer_Counter+=TWO_SECOND;
			...
			{	Send_SIP_Command(Command_Data[Cur].SIP_Msg.Command,&Command_Data[Cur].SIP_Msg,ADD_VIA,TO);
				...
				Close_Socket(Command_Data[Cur].Relay_TCP_Socket,m_hWnd);
				Command_Data[Cur].State=SIP_INVITE;  Command_Data[Cur].Timer_Counter=0;  
			}  
		}
		...
		{	Command_Data[Cur].Timer_Counter+=TWO_SECOND;
			...
			{	strcpy(Command_Data[Cur].Response_SIP_Msg.ViaBuf,Command_Data[Cur].SIP_Msg.ViaBuf);
				if (Command_Data[Cur].Abandon_Flag!=2)	Send_SIP_Command(Command_Data[Cur].Response_SIP_Msg.Command,&Command_Data[Cur].Response_SIP_Msg,DELETE_VIA,FROM);  
			}// 有些狀況不可 Delete_Via
			else
			{	Modify_CSeq(Command_Data[Cur].SIP_Msg.Cmd_Str,"ACK");
	...
	else if (nIDEvent>=TIMERID_SUBSERVER_EVENT && nIDEvent<(TIMERID_SUBSERVER_EVENT+MAPPING_DATA_SIZE))
	{
		...
		// Set SubServer Data
		Connect_Data.Flag=TRUE;
		Set_SubServer_Data(&Connect_Data,SubServer_Data[SubServer_Cur].IP,SubServer_Data[SubServer_Cur].Port);
		Update_SubServer_Data(&Connect_Data);
...
BOOL CSIP_Server::DestroyWindow() 
{
	...
	{
       for (i=0,Cur=0;Cur<SIP_ONLINE_SIZE;Cur++)
	   {   if (SIP_OnLine_Data[Cur].Flag) { fwrite(&SIP_OnLine_Data[Cur],sizeof(SIP_ONLINE_DATA),1,Out); i++; }
	       if (i>=SIP_OnLine_Num) { Cur=SIP_ONLINE_SIZE; break; }    }
	   fclose(Out);
	}
...
void CSIP_Server::OnRTP_Relay(WPARAM wParam,LPARAM lParam)
{
int          Cur,j;
char         S1[1024];
  ...
  { case FD_CONNECT:
         // FD_CONNECT: SIP_RELAY_CONNECT -> SIP_RELAY_READ
 	     if (HIWORD(lParam)==0)
		 {  if ( ((Cur=Check_Command_By_Relay_Socket(wParam))>=0) && (Command_Data[Cur].State==SIP_RELAY_CONNECT) )
		    {  Send_Relay_Command("0RL",&Command_Data[Cur].SIP_Msg,&Command_Data[Cur]);
			   Command_Data[Cur].State=SIP_RELAY_READ; Command_Data[Cur].Timer_Counter=0; }
			else  Close_Socket(wParam,m_hWnd);		 }
    ...
    case FD_READ:
		 j = Hsf_recv(wParam,S1,sizeof(S1)-1,0); if (j<=0) break; else S1[j]=0;
		 Cur = Check_Command_By_Relay_Socket(wParam);
	...
	case FD_CLOSE:
         ...
         if ((Cur=Check_Command_By_Relay_Socket(wParam))>=0) 
...
int CSIP_Server::Invite_Proc(int Send_Flag, SIP_MSG *lpSIP_Msg,SIP_STATE State,char *Command)
{	// 當Server端主動送封包時，需填入Command否則無法得知正確發送的Command
	int	Invite_Cur;
	SIP_INVITE_DATA	*lpInvite_Data;

	if ((Invite_Cur=Check_Invite(lpSIP_Msg))>=0)	lpInvite_Data = &SIP_Invite_Data[Invite_Cur];
	// Request Process
	if (Send_Flag==FROM)
	{	
		// Save Invite
		...
		{ 
			if (Invite_Cur<0)
			{	
				if ((Invite_Cur=Save_Invite(lpSIP_Msg))>=0)	lpInvite_Data = &SIP_Invite_Data[Invite_Cur];
				lpInvite_Data->Invite_Time = CTime::GetCurrentTime();
				sprintf(SIP_Invite_Data[Invite_Cur].Log_File_Name, "%s\\%s to %s\\%s",
					lpInvite_Data->Invite_Time.Format("%Y-%m-%d\\%H clock"),lpInvite_Data->From_PhoneNo,lpInvite_Data->To_PhoneNo,lpInvite_Data->Invite_Time.Format("%H-%M-%S"));
		...
		else if (Invite_Cur>=0)
		{
			...
			{
				if ((lpInvite_Data->Answer_State==Busy || lpInvite_Data->Answer_State==Forwarding) && State==SIP_200)	Clear_Invite(lpSIP_Msg);
				else if (lpInvite_Data->Answer_State==Inviting)
				{	lpInvite_Data->Start_Time = CTime::GetCurrentTime();
			...
			{ 	lpInvite_Data->End_Time = CTime::GetCurrentTime();
			...
			else 
			{
				if (lpInvite_Data->Answer_State==Inviting)
				{
					...
					else if (!strcmp(lpSIP_Msg->Command,"302"))	{ lpInvite_Data->Answer_State = Forwarding; Write_Invite_Log(lpInvite_Data, Forwarding);}
					else if (lpSIP_Msg->Command[0]=='4')
					{	
						if (!strcmp(lpSIP_Msg->Command,"486"))	{ Write_Invite_Log(lpInvite_Data, Busy); lpInvite_Data->Answer_State = Busy;}
						else
				...
				{
					if (!strcmp(lpSIP_Msg->Command,"200") && (State==SIP_BYE || State==SIP_CANCEL))		Clear_Invite(lpSIP_Msg);
					else if (!strcmp(lpSIP_Msg->Command,"481"))	Clear_Invite(lpSIP_Msg);
				}				
			}
		}
	}
	...
	{	// 用於當超過30秒Callee沒接電話,Server將會中止這通會話
		...
		{	
			...
			else if (lpInvite_Data->Answer_State==Busy || lpInvite_Data->Answer_State==Forwarding)	Clear_Invite(lpSIP_Msg);
		}
		// Response Process
		else 
		{
	...
	else if (Send_Flag==EXTRA && Invite_Cur>=0)	
	{
		...
		{
			if (State==SIP_BYE || State==SIP_CANCEL)	Clear_Invite(lpSIP_Msg);
		}
	}
	Show_Invite();
...
int CSIP_Server::Update_SIP_Server_Param(char *Method)
{
	...
	{
		GetPrivateProfileString("Setup Info","RTP_Relay_IP",m_IP,RTP_Relay_IP,sizeof(RTP_Relay_IP),SERVER_INFO_PATH);
		GetPrivateProfileString("Setup Info","RTP_Relay_Port",m_Port,RTP_Relay_Port,sizeof(RTP_Relay_Port),SERVER_INFO_PATH);
		GetPrivateProfileString("Setup Info","NTUT_GW_IP",m_NTUT_IP,NTUT_GW_IP,sizeof(NTUT_GW_IP),SERVER_INFO_PATH);
		GetPrivateProfileString("Setup Info","NTUT_GW_Port",m_NTUT_Port,NTUT_GW_Port,sizeof(NTUT_GW_Port),SERVER_INFO_PATH);
		GetPrivateProfileString("Setup Info","Outbound_PhoneNo",m_Outbound_PhoneNo,Outbound_PhoneNo,sizeof(Outbound_PhoneNo),SERVER_INFO_PATH);
		GetPrivateProfileString("Setup Info","Log_File",m_Log_File,Log_File,sizeof(Log_File),SERVER_INFO_PATH);
		GetPrivateProfileString("Setup Info","Search_PhoneNo",m_Number_Search,Search_PhoneNo,sizeof(Search_PhoneNo),SERVER_INFO_PATH);
		...
		sprintf(S1,"Relay Level %d",Relay_100_Flag); GetDlgItem(IDC_BUTTON3)->SetWindowText(S1);
		UpdateData(FALSE);
	...
	{	
		...
		UpdateData(TRUE);
		strcpy(RTP_Relay_IP,m_IP);
		strcpy(RTP_Relay_Port,m_Port);
		strcpy(NTUT_GW_IP,m_NTUT_IP); 
		strcpy(NTUT_GW_Port,m_NTUT_Port);
		strcpy(Outbound_PhoneNo,m_Outbound_PhoneNo);
		strcpy(Log_File,m_Log_File);
		strcpy(Search_PhoneNo,m_Number_Search);
		UpdateData(FALSE);

		WritePrivateProfileString("Setup Info","RTP_Relay_IP",m_IP,SERVER_INFO_PATH);
		WritePrivateProfileString("Setup Info","RTP_Relay_Port",m_Port,SERVER_INFO_PATH);
		WritePrivateProfileString("Setup Info","NTUT_GW_IP",m_NTUT_IP,SERVER_INFO_PATH);
		WritePrivateProfileString("Setup Info","NTUT_GW_Port",m_NTUT_Port,SERVER_INFO_PATH);
		WritePrivateProfileString("Setup Info","Outbound_PhoneNo",m_Outbound_PhoneNo,SERVER_INFO_PATH);
		WritePrivateProfileString("Setup Info","Log_File",m_Log_File,SERVER_INFO_PATH);
		WritePrivateProfileString("Setup Info","Search_PhoneNo",m_Number_Search,SERVER_INFO_PATH);
		sprintf(S1,"%d",Relay_100_Flag); WritePrivateProfileString("Setup Info","Relay_100_Flag",S1,SERVER_INFO_PATH);
	}
	return 0;
}

int CSIP_Server::Save_SubServer_Data(SUBSERVER_DATA *lpSubServer_Data)
{
	int SubServer_Cur;

	if ((SubServer_Cur=Check_SubServer_Data(lpSubServer_Data))<0)
	{
		for (SubServer_Cur=0;SubServer_Cur<SUBSERVER_DATA_SIZE;SubServer_Cur++) { if (!SubServer_Data[SubServer_Cur].Flag) break;}
		if (SubServer_Cur<SUBSERVER_DATA_SIZE)
		{	
			memcpy(&SubServer_Data[SubServer_Cur],lpSubServer_Data,sizeof(SUBSERVER_DATA));
			SubServer_Data[SubServer_Cur].Flag=TRUE;
			SubServer_Num++;
		}
		else SubServer_Cur=SUBSERVER_DATA_SIZE;
	}
	else
	{
		memcpy(&SubServer_Data[SubServer_Cur],lpSubServer_Data,sizeof(SUBSERVER_DATA));
	...
	if (SubServer_Cur<SUBSERVER_DATA_SIZE)	return SubServer_Cur; else return -1;
}

int CSIP_Server::Check_SubServer_Data(SUBSERVER_DATA *lpSubServer_Data)
{
	int		i,SubServer_Cur;
	...
	if (SubServer_Num>0 && SubServer_Num<=SUBSERVER_DATA_SIZE)	
	{
		...
		{ if ( SubServer_Data[SubServer_Cur].Flag && 
			 (!strcmp(SubServer_Data[SubServer_Cur].IP,lpSubServer_Data->IP) && !strcmp(SubServer_Data[SubServer_Cur].Port,lpSubServer_Data->Port)) )	break;
			i++; if (i>=SubServer_Num)	{ SubServer_Cur=SUBSERVER_DATA_SIZE; break;}
		}
	}else SubServer_Cur=SUBSERVER_DATA_SIZE;
	if (SubServer_Cur<SUBSERVER_DATA_SIZE) return SubServer_Cur; else return -1;
}

int CSIP_Server::Update_SubServer_Data(SUBSERVER_DATA *lpSubServer_Data)
{
	...
	if (Mapping_Num>0 && Mapping_Num<=MAPPING_DATA_SIZE)
	{
		...
		{   
			...
			{  	
				for (Cur=0;Cur<SERVER_PORT_NUM;Cur++)	if (Mapping_Data[Mapping_Cur].SubServer_Data[Cur].Flag)
				{
					if ( !strcmp(Mapping_Data[Mapping_Cur].SubServer_Data[Cur].IP,lpSubServer_Data->IP) &&
						 !strcmp(Mapping_Data[Mapping_Cur].SubServer_Data[Cur].Port,lpSubServer_Data->Port) )
					{
						memcpy(&Mapping_Data[Mapping_Cur].SubServer_Data[Cur],lpSubServer_Data,sizeof(SUBSERVER_DATA));
						Mapping_Data[Mapping_Cur].SubServer_Data[Cur].Flag=TRUE;
					}
				}
				// Calculate i
				i++; if (i>=Mapping_Num) { Mapping_Cur=MAPPING_DATA_SIZE; break;}
			}
		}
	}else Mapping_Cur=MAPPING_DATA_SIZE;
	if (Mapping_Cur>=MAPPING_DATA_SIZE)	return -1;
...
{
	memset(&SubServer_Data[SubServer_Cur],0,sizeof(SUBSERVER_DATA));
	return 0;
}
/*
int	CSIP_Server::Access_OnLine_State(SIP_MSG *lpSIP_Msg)
{
	int			Invite_Cur,Online_Cur;
	LUS_DATA	LUS_Data;
	char		Buf[100];
	ANSWER_STATE	Answer_State;

	// Initial
	memset(&LUS_Data,0,sizeof(LUS_DATA));
	
	if ((Online_Cur=Check_OnLine(lpSIP_Msg->From_Name))>=0)
	{
		// Set LUS_Data
		strncpy(LUS_Data.szPhoneNo,SIP_OnLine_Data[Online_Cur].PhoneNo,sizeof(LUS_Data.szPhoneNo)-1);
		sprintf(Buf,"%s/ %s",SIP_OnLine_Data[Online_Cur].R_IP,SIP_OnLine_Data[Online_Cur].V_IP);
		strncpy(LUS_Data.szIP,Buf,sizeof(LUS_Data.szIP)-1);
		sprintf(Buf,"%d/ %d",SIP_OnLine_Data[Online_Cur].S_Port,SIP_OnLine_Data[Online_Cur].Server_Port);
		strncpy(LUS_Data.szPort,Buf,sizeof(LUS_Data.szPort)-1);
		sprintf(Buf,"%d/ %d/ %d/ %d",SIP_OnLine_Data[Online_Cur].Expires_Client,SIP_OnLine_Data[Online_Cur].Expires_Real,SIP_OnLine_Data[Online_Cur].Expires_Limit,SIP_OnLine_Data[Online_Cur].Expires_Counter);
		strncpy(LUS_Data.szExpires,Buf,sizeof(LUS_Data.szExpires)-1);
		if (SIP_OnLine_Data[Online_Cur].CDMS_OK==1)	strncpy(LUS_Data.szAccount,"OK",sizeof(LUS_Data.szAccount)-1);
		else if (SIP_OnLine_Data[Online_Cur].CDMS_OK==0) sprintf(LUS_Data.szAccount,"unCheck",sizeof(LUS_Data.szAccount)-1); 
		else if (SIP_OnLine_Data[Online_Cur].CDMS_OK==-1) sprintf(LUS_Data.szAccount,"Fail",sizeof(LUS_Data.szAccount)-1); 
		strncpy(LUS_Data.szPassword,(SIP_OnLine_Data[Online_Cur].MD5_OK)? "OK":"Fail",sizeof(LUS_Data.szPassword)-1);
		strncpy(LUS_Data.szProxy,(SIP_OnLine_Data[Online_Cur].Proxy_MD5_OK)? "OK":"Fail",sizeof(LUS_Data.szProxy)-1);
		strncpy(LUS_Data.szName,SIP_OnLine_Data[Online_Cur].Name,sizeof(LUS_Data.szName)-1);
		
		// Invite State
		if ((Invite_Cur=Check_Invite(lpSIP_Msg))>=0)
		{	
			Answer_State=SIP_Invite_Data[Invite_Cur].Answer_State;
			strcpy(LUS_Data.szState,"Normal");
			if (Answer_State==Inviting || Answer_State==Answer || Answer_State==Abandon || Answer_State==Busy || Answer_State==Forwarding)	
			{ 
				strcpy(LUS_Data.szState,"Answered");
				strcpy(LUS_Data.szCalleeId,SIP_Invite_Data[Invite_Cur].To_PhoneNo);
			}
		}else	strcpy(LUS_Data.szState,"Normal");
		
		// Add new User
//		if (lpCDMS_LUS->EditLus(&LUS_Data)==0)	lpCDMS_LUS->AddLus(&LUS_Data);
...
int CSIP_Server::Write_Log(char *Head,char *S1,sockaddr_in *lpAddr,SIP_MSG *lpSIP_Msg)
{
	CTime Time1;
	SYSTEMTIME TimeDest;
	FILE  *HsfOut1;
	...
	char  S2[120];
	int   i;
		  for (i=0;i<70;i++) if ((S1[i]==0x0a)||(S1[i]==0x0d)) break;
		  memcpy(S2,S1,i); S2[i]=0;
		  if (lpSIP_Msg!=NULL) TRACE("%s:  %s Expire: %s\r\n",Head,S2,lpSIP_Msg->Expires);
		  else                 TRACE("%s:  %s\r\n",Head,S2);
	#endif     
...
//----------------------------------------------------------------------------------------------------
	if (!Write_Flag) return -1;

	CRYPTO_TYPE	Crypto_Type;
	char		szCrypto_Type[20];

	// 寫入 Log 檔案
	if ( (strlen(Log_File)>0) && (strlen(Log_File)<20) ) sprintf(Name,"C:/SIP_Log_%s.txt",Log_File); else strcpy(Name,LOG_FILE_NAME);
	if (strlen(Log_File)>0) 
	{  
		if ( (lpSIP_Msg==NULL)/* || (lpAddr==NULL) */) return -1;
 		if ( (strcmp(Log_File,lpSIP_Msg->From_Name)) && (strncmp(Log_File,lpSIP_Msg->To_Name,strlen(Log_File))) ) return -1;  
	}
	Get_Network_Mothod(Head,lpSIP_Msg,Crypto_Type);
	Time1=CTime::GetCurrentTime(); GetSystemTime(&TimeDest);
	if ( (HsfOut1=fopen(Name,"a+"))==NULL ) return -1;
	else
	{  
		...
		// Crypto Type
		if		(Crypto_Type==CT_NoCrypto)		strcpy(szCrypto_Type,"NoCrypto");
		else if (Crypto_Type==CT_SIPCrypto)		strcpy(szCrypto_Type,"SIP_Crypto");
		else if (Crypto_Type==CT_Walkersun1)	strcpy(szCrypto_Type,"Walkersun1_Crypto");
		else									strcpy(szCrypto_Type,"Crypto_Error");
...
int CSIP_Server::Get_Network_Mothod(const char *Head,const SIP_MSG *lpSIP_Msg,CRYPTO_TYPE &Crypto_Type)
{
	...
	if (lpSIP_Msg!=NULL)
	{
		if (!strcmp(Head,HEAD_RECVFROM) || !strcmp(Head,HEAD_SENDTO))
		{
			...
			Crypto_Type=(Temp_Flag)? lpSIP_Msg->From_Crypto_Type:lpSIP_Msg->To_Crypto_Type;
		}else	{ Crypto_Type=CT_NoCrypto;}
	}else	return -1;
	return 0;
}
...
int  CSIP_Server::Init_Monitor()
{
	...
	{  SetWindowText("TCP Server for Monitor is Fail"); }
...
int  CSIP_Server::Init_Command()
{
	...
	{	Command_Data[Cur].State=SIP_READY;
		Command_Data[Cur].Abandon_Time = ABANDON_TIME;
		//Command_Data[Cur].Forwarding_GW_Flag=FALSE; 
	}
	 return 0;
}

int  CSIP_Server::Check_Command_By_Relay_Socket(SOCKET Socket)
{
int  i,Cur;

     // 2. Check Exist Command
	 for (i=0,Cur=0;Cur<COMMAND_QUEUE_SIZE;Cur++)
	 {   if (Command_Queue_Num==0) { Cur=COMMAND_QUEUE_SIZE; break; } 
	     if (Command_Data[Cur].State!=SIP_READY)
	     {  if (Command_Data[Cur].Relay_TCP_Socket==Socket) break;
		    i++; if (i>=Command_Queue_Num) { Cur=COMMAND_QUEUE_SIZE; break; }
	 }	 }   
	 // 3. Return Result
	 if (Cur>=COMMAND_QUEUE_SIZE) return -1;
	 else                         return Cur;
}
...
char       Command[100];
	 ...
	 {   if (Command_Queue_Num==0) { Cur=COMMAND_QUEUE_SIZE; break; } 
	     if (Command_Data[Cur].State!=SIP_READY)
		 {  // 1. Check Command
            if ( (lpSIP_Data->Command[0]>='1') && (lpSIP_Data->Command[0]<='4') ) Get_CSeq_Num(lpSIP_Data->Cmd_Str,"CSeq:",Command);
			else                                                                  strcpy(Command,lpSIP_Data->Command);
            // 2. Check From, To
	 ...
	 if (Cur<COMMAND_QUEUE_SIZE) return Cur; else return -1;
}
...
int  CSIP_Server::Save_Command(SIP_MSG *lpSIP_Data)
{
	...
	for (Command_Cur=0;Command_Cur<COMMAND_QUEUE_SIZE;Command_Cur++) { if (Command_Data[Command_Cur].State==SIP_READY) break; }
	if (Command_Cur<COMMAND_QUEUE_SIZE)
	{	
		if		( !strcmp(lpSIP_Data->Command,"REGISTER") )	Command_Data[Command_Cur].State=SIP_REG;	
		else if ( !strcmp(lpSIP_Data->Command,"CANCEL") )	Command_Data[Command_Cur].State=SIP_CANCEL;
	    else if ( !strcmp(lpSIP_Data->Command,"BYE") )		Command_Data[Command_Cur].State=SIP_BYE;
	    else if ( !strcmp(lpSIP_Data->Command,"REFER") )	Command_Data[Command_Cur].State=SIP_REFER;
		else if ( !strcmp(lpSIP_Data->Command,"NOTIFY") )	Command_Data[Command_Cur].State=SIP_NOTIFY;
        else if ( !strcmp(lpSIP_Data->Command,"ACK") )		return -1;
...
{   
	if (Command_Data[Cur].State==SIP_READY) return -1;
	else  
	{	memset(&Command_Data[Cur],0,sizeof(COMMAND_QUEUE));
		KillTimer(TIMERID_COMMAND+Cur);
		Command_Data[Cur].State=SIP_READY; Command_Queue_Num--; return 0; 
	}
}

int CSIP_Server::Update_Command(int Cur,char *Command)
{
	int  i,j;
	char Buf[200],Buf1[100],S1[SIP_MSG_LEGNTH];
    ...
    if ( (Command_Data[Cur].State!=SIP_RELAY_CONNECT) &&
		 ...
		 (Command_Data[Cur].State!=SIP_200) ) return -1;
	else
	{  Command_Data[Cur].State=SIP_200; 
	   ...
	   if (strlen(Command_Data[Cur].Response_SIP_Msg.Command)<=0) memcpy(&Command_Data[Cur].Response_SIP_Msg,&Command_Data[Cur].SIP_Msg,sizeof(SIP_MSG));
	   ...
	   for (j=i;j<(int)strlen(S1);j++) if ((S1[j]!=0x0a)&&(S1[j]!=0x0d)) break;
	   strcpy(S1,Buf); strcat(S1,&Command_Data[Cur].Response_SIP_Msg.Cmd_Str[j]);
...
{   
    if (Command_Data[Cur].State!=SIP_READY) memcpy(&Command_Data[Cur].SIP_Msg,lpSIP_Msg,sizeof(SIP_MSG)); return 0;
}
int CSIP_Server::Update_Command_By_Response(int Cur,SIP_MSG *lpSIP_Msg)
{   
		memcpy(&Command_Data[Cur].Response_SIP_Msg,lpSIP_Msg,sizeof(SIP_MSG)); return 0;
}
...
int  CSIP_Server::Init_OnLine()
{
	...
	SIP_OnLine_Num=0;
	for (i=0;i<SIP_ONLINE_SIZE;i++) SIP_OnLine_Data[i].Flag=FALSE;
	memset(SIP_OnLine_Data,0,sizeof(SIP_OnLine_Data));
    SetTimer(TIMERID_EXPIRES,ONE_SECOND,NULL);

	return 0;
}

int  CSIP_Server::Check_OnLine(char *PhoneNo,sockaddr_in *Addr)
{
	int		i,Cur;
	bool	LogicA,LogicB;

	// Search PhoneNo Exist or not ?
	if ( (strlen(PhoneNo)>0) && (SIP_OnLine_Num>0) )
	{  	for (i=0,Cur=0;Cur<SIP_ONLINE_SIZE;Cur++)
		{   if (SIP_OnLine_Data[Cur].Flag)
			{	if (!strcmp(SIP_OnLine_Data[Cur].PhoneNo,PhoneNo))
				{	if (Addr!=NULL)
					{	LogicA=Addrcmp(Addr,&SIP_OnLine_Data[Cur].Trunk_GW.Addr);
						LogicB=Addrcmp(&SIP_OnLine_Data[Cur].Addr,&SIP_OnLine_Data[Cur].Trunk_GW.Addr);
						if ((!LogicA&&!LogicB)||(LogicA&&LogicB))	break;
					}
					// Logic Operator 
					else if (Addrcmp(&SIP_OnLine_Data[Cur].Addr,&SIP_OnLine_Data[Cur].Trunk_GW.Addr))	break;
				}	
				i++; if (i>=SIP_OnLine_Num) { Cur=SIP_ONLINE_SIZE; break;}
			}
		}
		if (Cur>=SIP_ONLINE_SIZE) return -1; else return Cur;	 
	}
	else return -1;
}
...
int  CSIP_Server::Save_OnLine(SIP_MSG *lpData,int Flag)
{
	...
	// Error Checking
	if (strlen(SS1)<1) return -2;	// 號碼長度不能小於1
	else if (strlen(SS1)>30)	return -3;	// 號碼長度不能大於30
	for (Cur=0;Cur<(int)strlen(SS1);Cur++) if (((SS1[Cur]<'0')||(SS1[Cur]>'9'))&&(SS1[Cur]!='-')) return -4;

	// 2. OffLine
	if ((Cur=Check_OnLine(SS1,lpAddr))<0)
	{	if (SIP_OnLine_Num>=SIP_ONLINE_SIZE) return -1;
		for (Cur=0;Cur<SIP_ONLINE_SIZE;Cur++) if (!SIP_OnLine_Data[Cur].Flag) break;
		if (Cur>=SIP_ONLINE_SIZE) return -1;
		else
		{	SIP_OnLine_Num++;
			...
			// Get Server Port
			for (i=0;i<Server_Port_Num;i++)	if (SIP_Server_Socket[i]==lpData->FSocket)	{ SIP_OnLine_Data[Cur].Server_Port=SIP_Server_Port[i]; break;}
...
void CSIP_Server::Set_OnLine(SIP_MSG *lpData,int Cur,int Flag)
{
	...
	if (Flag==FROM)
	{	// 儲存 OnLine 基本資料	  
		SIP_OnLine_Data[Cur].Socket=lpData->FSocket;
		Port=lpData->FAddr.sin_port; SIP_OnLine_Data[Cur].S_Port=((Port&0xFF)<<8)+((Port>>8)&0xFF);
	...
	{	// 被叫端未註冊過, 首次被 INVITE(PC to Phone or Co-Server), 需將之存入 OnLine Buffer
		SIP_OnLine_Data[Cur].Socket=lpData->TSocket;
		Port=lpData->TAddr.sin_port; SIP_OnLine_Data[Cur].S_Port=((Port&0xFF)<<8)+((Port>>8)&0xFF);
...
int CSIP_Server::Check_CDMS_Data(const char *Account)
{
	...
	{   
		if (CDMS_Data_Num==0) { Cur=CDMS_DATA_SIZE; break; } 
		if (CDMS_Data[Cur].PhoneNo[0])
		{  
			// Check From, To
			if (!strcmp(CDMS_Data[Cur].PhoneNo,Account))   break;
			i++; if (i>=CDMS_Data_Num) { Cur=CDMS_DATA_SIZE; break; }
		}
	}
	if (Cur<CDMS_DATA_SIZE) return Cur; else return -1;
	return 0;
}
...
int CSIP_Server::Save_CDMS_Data(const CDMS_DATA *lpCdms)
{
	...
	if (!lpCdms)	return -1;
	for (Cur=0;Cur<CDMS_DATA_SIZE;Cur++) { if (!CDMS_Data[Cur].PhoneNo[0]) break; }
...
{
	if (Cur<0 || Cur>=CDMS_DATA_SIZE)	return -1;
	if (CDMS_Data[Cur].PhoneNo[0])
	{
...
int CSIP_Server::Check_MD5(SIP_MSG *lpData,int Online_Cur,bool Access_Flag)
{
	...
	if (SIP_OnLine_Data[Online_Cur].CDMS_OK==0 || Access_Flag)
	{
		...
		else SIP_OnLine_Data[Online_Cur].CDMS_OK=-1;
	}
	else strcpy(Password,SIP_OnLine_Data[Online_Cur].Password);
	...
	{  
		...
		{   
			...
			{	if (Reg_Auth_Level==2||Reg_Auth_Level==4)	strcpy(lpData->Authorization_Nonce,SIP_OnLine_Data[Online_Cur].Nonce);
			...
			{	if (Proxy_Auth_Level==3)	strcpy(lpData->Authorization_Nonce,SIP_OnLine_Data[Online_Cur].Proxy_Nonce);
				MD5(lpData->Authorization_Algorithm,lpData->Command,lpData->Authorization_Username,lpData->Authorization_Uri,Password,lpData->Authorization_Realm,lpData->Authorization_Nonce,Response);
...
char S1[100],SS1[SIP_MSG_LEGNTH];
    ...
    if (0<Step&&Step<MIN_EXPIRES) Step=MIN_EXPIRES; else if (Step>MAX_EXPIRES) Step=MAX_EXPIRES;
    sprintf(S1,"%d",Step); strcpy(lpData->Expires,S1);
    if ( (i=Get_Position(lpData->Cmd_Str,"Expires:")) > 0 )
	{  for (j=i;j<(int)strlen(lpData->Cmd_Str);j++) if ((lpData->Cmd_Str[j]>=0x30)&&(lpData->Cmd_Str[j]<=0x39)) break;
       for (k=j;k<(int)strlen(lpData->Cmd_Str);k++) if ((lpData->Cmd_Str[k]<0x30)||(lpData->Cmd_Str[j]>0x39)) break;
       strncpy(SS1,lpData->Cmd_Str,j); SS1[j]=0;
       strcat(SS1,S1);
...
{
	if (SIP_OnLine_Data[Online_Cur].First_Flag)	SIP_OnLine_Data[Online_Cur].Expires_Real=0;
	// 使用 REGISTER 註冊
	if (!strcmp(lpData->Command,"REGISTER"))
	{	
		...
		if (SIP_OnLine_Data[Online_Cur].MD5_OK)	SIP_OnLine_Data[Online_Cur].Expires_Limit=SIP_OnLine_Data[Online_Cur].Expires_Client*Expire_Limit_Weight;
	}
	else
	{	SIP_OnLine_Data[Online_Cur].Expires_Counter=0;
...
int CSIP_Server::Proc_Auth(SIP_MSG *lpSIP_Msg, int Command_Cur, int Online_Cur, char *Command)
{	
	...
	sprintf(Realm,"%s",lpSIP_Msg->FR_IP);
	sprintf(Nonce,"%luntut%luhsf%lu",rand()^0xA756,rand(),Time.GetTime()^GetTickCount());
	if (!strcmp(Command,"401"))
...
int  CSIP_Server::Clear_OnLine(SIP_MSG *lpSIP_Data)
{
	...
	// 2. Clear OnLine Data
	if (Clear_OnLine_By_Cur(Cur)>=0) return 0;
	else   return -1;
}
...
int  CSIP_Server::Clear_OnLine_By_Cur(int Cur)
{
	...
	{  
		// Clear Online_Data
		memset(&SIP_OnLine_Data[Cur],0,sizeof(SIP_ONLINE_DATA));
		SIP_OnLine_Num--;  
...
// ====================================================================================
int CSIP_Server::Extract_Param(char *S1,char *Head,char *Param)
{
int  i,j,Beg,End;
	 ...
	 {   if ( (S1[i]|0x20) == (Head[0]|0x20) ) { for (j=0;j<(int)strlen(Head);j++) if ( (S1[i+j]|0x20) != (Head[j]|0x20) ) break; }
		 if (j>=(int)strlen(Head)) break;   }

	 // 2. Locate Beg/End of Param
	 if ( i< (int)(strlen(S1)-strlen(Head)) )
	 {  Beg=i+(int)strlen(Head);
	    for (i=Beg;i<(int)strlen(S1);i++) if (S1[i]!=' ') { Beg=i; break; }
		for (i=Beg;i<(int)strlen(S1);i++) if ((S1[i]==0x20)||(S1[i]==0x0a)||(S1[i]==0x0d)) { End=i-1; break; }	 }

	 // 3. Extract Param
     if (End>=Beg)
	 {  for (i=Beg;i<=End;i++) if ( ((S1[i]<'0')||(S1[i]>'9')) && (S1[i]!='.') ) break;
	    if (i<=End) return -1;
		else		  
		{  strncpy(Param,&S1[Beg],End-Beg+1); Param[End-Beg+1]=0; }  }
	 else return -1;

	 return 0;
}

int CSIP_Server::Modify_Param(char *S1,char *Head,char *Param)
{
char SS1[SIP_MSG_LEGNTH];
int  i,Beg,End;
	 ...
	 // 3. Replace Param
     if (End>=Beg)
	 {  for (i=Beg;i<=End;i++) if ( ((S1[i]<'0')||(S1[i]>'9')) && (S1[i]!='.') && (S1[i]!='-') ) break; // '-' 乃針對 Golden-Net 而設計
	    if (i<=End) return -1;
		else		  
		{  strncpy(SS1,S1,Beg); SS1[Beg]=0;
		   strcat(SS1,Param); 
		   strcat(SS1,&S1[End+1]); 
		   strcpy(S1,SS1); }	 }
	 else return -1;
...
char S1[SIP_MSG_LEGNTH],Buf[100],Port[16];
	...
	if ((k=Get_Position(lpData->SDP,"c=IN IP4"))>0)	i=Modify_Param(&lpData->SDP[k+strlen("c=IN IP4")],"c=IN IP4",IP);
	// 支援新舊版，INVITE時TPort都不為"",表示為新版取TPort,其他情形皆取FPort
	if (!strcmp(lpData->Command,"INVITE") && strcmp(TPort,""))	strcpy(Port,TPort); else strcpy(Port,FPort);
	i=Modify_Param(lpData->SDP,"m=audio",Port);
	if (i<0) i=Modify_Param(lpData->SDP,"m=image",Port);
    Old_Len-=strlen(lpData->SDP);
	...
	{  for (j=i;j<(int)strlen(lpData->Cmd_Str);j++) if ((lpData->Cmd_Str[j]==0x0a)||(lpData->Cmd_Str[j]==0x0d)) break;
	   strncpy(S1,lpData->Cmd_Str,i); S1[i]=0;
	   strcat(S1,Buf);
...
void CSIP_Server::Send_SIP_Command(char *Command,SIP_MSG *lpSIP_Data,int Via_Flag,int Send_Flag)
{
	int				i,j,k,l,n,Len,Msg_Len;
	CTime			Time1; 
	char			S1[SIP_MSG_LEGNTH],Buf[256],Buf1[100],SS2[300],Msg=NULL,TPort[10],URL[200];
	WSADATA			wsadata;
	...
	if (Send_Flag==FROM)
	{  
		lpSIP_Data->Socket=lpSIP_Data->FSocket;
		memcpy(&lpSIP_Data->Addr,&lpSIP_Data->FAddr,sizeof(lpSIP_Data->FAddr)); 
		Crypto_Type=lpSIP_Data->From_Crypto_Type;
	}
	else if (Send_Flag==TO)
	{
		lpSIP_Data->Socket=lpSIP_Data->TSocket;
		memcpy(&lpSIP_Data->Addr,&lpSIP_Data->TAddr,sizeof(lpSIP_Data->TAddr)); 
		Crypto_Type=lpSIP_Data->To_Crypto_Type;
	}
	else	lpSIP_Data->Socket=NULL;
	...
	{	// Get Server Port
		...
		getsockname( lpSIP_Data->TSocket,(sockaddr *)&Server_Addr,&Len);
		for (n=0;n<Server_Port_Num;n++)	if (SIP_Server_Port[n]==ntohs(Server_Addr.sin_port))	break;
		...
		{   // URL 內涵port需將其去除
		   for (n=0;n<(int)strlen(lpSIP_Data->Command_URL);n++) if (lpSIP_Data->Command_URL[n]==':')	{strncpy(URL,lpSIP_Data->Command_URL,n-1); URL[n]=0; break;}
		   sprintf(URL,"%s:%s",URL,TPort);
		}
		// Update Command Line
		if (!strcmp(lpSIP_Data->Command_Name,""))	sprintf(Buf,"%s sip:%s SIP/2.0\r\n",Command,URL);	
		else sprintf(Buf,"%s sip:%s@%s SIP/2.0\r\n",Command,lpSIP_Data->Command_Name,URL);	
	}
	strcpy(S1,lpSIP_Data->Cmd_Str);
	if ( (j=Get_NextLine(S1))>0) strcpy(S1,Buf); strcat(S1,&lpSIP_Data->Cmd_Str[j]);	
	...
	{  // Remove SDP
	   ...
	   {  if ( (i=Get_Position(S1,"v=")) > 0 ) S1[i]=0; 
	      // Remove Content-Type:
	      if ( (i=Get_Position(S1,"Content-Type:")) > 0 )
		  {  if ( (k=Get_NextLine(&S1[i])) > 0 )
			 {  k+=i; Deletion(S1,i,k-1); }  }
	      // Set Content-Length: 0
	      if ( (i=Get_Position(S1,"Content-Length:")) > 0 )
		  {  for (j=i;j<(int)strlen(S1);j++) if (S1[j]==0x20) break;
	         for (k=j;k<(int)strlen(S1);k++) if ((S1[k]>='0')&&(S1[k]<='9')) break;
		     for (l=k;l<(int)strlen(S1);l++) if ((S1[l]<'0')||(S1[l]>'9')) break;
			 Deletion(S1,k,l-1);
			 Insertion(S1,k,"0");  }
	   }
	}
	...
	{  for (j=i;j<(int)strlen(S1);j++) if (S1[j]=='@') { j++; break; }
	   for (k=j;k<(int)strlen(S1);k++) if ((S1[k]=='>')||(S1[k]==0x0a)||(S1[k]==0x0d)) { break; }
	...
	{  Delete_Record_Route(S1); }//   不可清除 Record-Route: 以免 ACK 回錯位�
	...
	{	
		if ((i=Get_Position(S1,"Contact:"))<0)
		{
			...
			{
				if (strcmp(lpSIP_Data->Contact_Port,""))	sprintf(SS2,"Contact: <sip:%s@%s:%s>\r\n",lpSIP_Data->Contact_Name,Local_IP,lpSIP_Data->Contact_Port);
				else sprintf(SS2,"Contact: <sip:%s@%s>\r\n",lpSIP_Data->Contact_Name,Local_IP);
				Insertion(S1,i,SS2);
			}
		}else	Edit_URL(S1,"Contact:",Local_IP);
	}

	// 3. Send to Remote
	if (lpSIP_Data->Socket!=NULL) 
	{
		memcpy(&Temp_SIP_Msg,lpSIP_Data,sizeof(SIP_MSG)); strcpy(Temp_SIP_Msg.Command,Command);
		Write_Log("sendto",S1,&lpSIP_Data->Addr,&Temp_SIP_Msg);
		// Msg Crypt Process
		if (Crypto_Type==CT_SIPCrypto)	Msg_Len=sip_encrypt(S1,(unsigned char *)S1);
		else if (Crypto_Type==CT_Walkersun1)	Msg_Len=walkersun_sip_encrypt(S1,(unsigned char *)S1);
		else	Msg_Len=strlen(S1);
		i=sendto(lpSIP_Data->Socket,S1,Msg_Len,0,(sockaddr *)&lpSIP_Data->Addr,sizeof(sockaddr));  
	}  
}
...
int CSIP_Server::Add_Via(char *S1)
{
	...
	sprintf(Buf,"Via: SIP/2.0/UDP %s:%s;branch=z9hG4bK%lu%lu\r\n",Local_IP,LPort,rand(),rand());

    if ( (i=Get_Position(S1,"Via:")) <= 0 ) i=Get_NextLine(S1);
	if (i>=0) Insertion(S1,i,Buf); else return -1;
	return 0;
}
...
int CSIP_Server::Delete_Record_Route(char *S1)
{
   ...
   if ( (i=Get_Position(S1,"Record-Route:")) > 0 )   {  if ( (k=Get_NextLine(&S1[i])) > 0 ) {  k+=i; Deletion(S1,i,k-1); }  }
   return 0;
}
...
// -----------------------------------------------------------------
int CSIP_Server::SIP_Message_Analyzer(SOCKET wParam,sockaddr_in *lpAddr,char *S1,SIP_MSG *Data,CRYPTO_TYPE Crypto_Type)
{
    ...
    // 1. Get SIP_Command
    for (i=0;i<SIP_COMMAND_NO;i++) { if (!strncmp(&SIP_COMMAND[i][0],S1,strlen(&SIP_COMMAND[i][0]))) { Cmd=i; break; }	}

	// 2. Get SIP Parameter
	if (Cmd<SIP_COMMAND_NO)
	{	// 2.1 Get Response_Code/Request Command
		if (Cmd==SIP_COMMAND_NO-1) 
		{ sscanf(S1,"%s %s %s",S2,Data->Command,S2); Data->ViaBuf[0]=0;  }
		else                     
		...
		if (atoi(Data->Expires)==0 && strcmp(Data->Expires,"0"))	strcpy(Data->Expires,Data->Contact_expires);	//有些UA沒有Expries:,其expries被放在Contact:裡面
		Get_Field(S1,"Content-Length:",Data->Content_Length);
		if ( atoi(Data->Content_Length) >0 ) Get_SDP(S1,Data->SDP);
...
int CSIP_Server::Transform_PhoneNo(SIP_MSG *lpSIP_Msg, bool Before_Flag)
{
	...
	if (!strcmp(lpSIP_Msg->Command,"INVITE"))
	{	
		if ((Invite_Cur=Check_Invite(lpSIP_Msg))<0)	Invite_Proc(FROM, lpSIP_Msg, SIP_READY); 
		else if (!Before_Flag) Save_Invite(lpSIP_Msg);	// Set Param (TR_IP, TV_IP...)
	}
		...
		{	
			...
			case 2:	// Response and <--
				...
				{	Edit_Name(lpSIP_Msg->Cmd_Str,"To:",lpInvite_Data->Partial_PhoneNo);
					if (Get_Position(lpSIP_Msg->Cmd_Str,"Contact:")>=0)	Edit_Name(lpSIP_Msg->Cmd_Str,"Contact:",lpInvite_Data->Partial_PhoneNo);
					Transform_Flag=TRUE;
				}
				break;
			case 3: // Response and -->
				if (Before_Flag)
				{
					...
					if (Get_Position(lpSIP_Msg->Cmd_Str,"Contact:")>=0)	Edit_Name(lpSIP_Msg->Cmd_Str,"Contact:",lpInvite_Data->Complete_PhoneNo);
					Transform_Flag=TRUE;
				}
				break;
			}
		}
		else	Transform_Flag=FALSE;
	}
	if (Transform_Flag==TRUE) return 0; else return -1;
}
	...
	if ((Invite_Cur=Check_Invite(lpSIP_Msg))>=0)
	{
		if (strlen(SIP_Invite_Data[Invite_Cur].Partial_PhoneNo)<=0)
		{
			...
			{	
				...
				{
					// Get From Name
					if ((Start_Cur=strchr(lpSIP_Msg->From_Name,'-'))!=NULL)	{ strncpy(From_Name,lpSIP_Msg->From_Name,Start_Cur-lpSIP_Msg->From_Name); From_Name[Start_Cur-lpSIP_Msg->From_Name]=0;}
					else	strcpy(From_Name,lpSIP_Msg->From_Name);
					
					// From_Name屬於10碼區: (02),(04); To_Name 5~8碼
					if		((From_Name[0]=='0' && (From_Name[1]=='2'||From_Name[1]=='4')) && strlen(From_Name)==10 && strlen(lpSIP_Msg->To_Name)<=8)	Type = 1;
					// From_Name屬於9碼區: (03), (05), (06), (07), (08); To_Name 5~7碼
					else if ((From_Name[0]=='0' && (From_Name[1]>='1'&&From_Name[1]<='8')) && strlen(From_Name)==9 && strlen(lpSIP_Msg->To_Name)<=7)	Type = 2;
					// From_Name屬於大哥大號碼: 09XX...; To_Name 5~8碼
					else if ((From_Name[0]=='0' && From_Name[1]=='9') && strlen(From_Name)==10 && strlen(lpSIP_Msg->To_Name)<=8)	Type = 3;
					// From_Name為8610, 8660, etc..開頭; To_Name 5~8碼
					else if ((From_Name[0]>='1' && From_Name[0]<='9') && strlen(From_Name)==10 && strlen(lpSIP_Msg->To_Name)<=8)	Type = 4;
					// 其他
					else	Type = 9;
					if (Type!=9 && Start_Cur!=NULL)	Type+=4;
					...
					case 1:	case 2:	case 3:	case 4:
						...
						strncpy(PhoneNo,From_Name,Len); PhoneNo[Len]=0;
						strcat(PhoneNo,lpSIP_Msg->To_Name);
						break;
					case 5: case 6: case 7: case 8:
						Filter_Flag=TRUE;
						// 若To_Name是四碼則表示撥分機，未來可擴通至5碼
						if (strlen(lpSIP_Msg->To_Name)==4)	Start_Cur=&lpSIP_Msg->From_Name[strlen(lpSIP_Msg->From_Name)];
						Len=(Start_Cur-lpSIP_Msg->From_Name)-strlen(lpSIP_Msg->To_Name);
						strncpy(PhoneNo,lpSIP_Msg->From_Name,Len); PhoneNo[Len]=0;
						strcat(PhoneNo,lpSIP_Msg->To_Name);
						break;
					case 9:
						Filter_Flag=FALSE;
...
int CSIP_Server::OutBound_Transform_PhoneNo(SIP_MSG *lpSIP_Msg, bool Before_Flag)
{
	...
	if (strlen(Outbound_PhoneNo)<=0)	return -1;
	...
	{	
		...
		{	
			...
			case 1:	// Request and <--
				if (Before_Flag)
				{	// 去除To_Name之總機號碼
					for (i=0;i<(int)strlen(lpSIP_Msg->To_Name);i++) if (lpSIP_Msg->To_Name[i]=='-') break;
			...
			case 2:	// Response and <--
				...
				{	// PC to Phone: 去除 總機號碼 0227712171-2178
					for (i=0;i<(int)strlen(lpSIP_Msg->From_Name);i++) if (lpSIP_Msg->From_Name[i]=='-') break;
...
bool CSIP_Server::OutBound_Filter(SIP_MSG *lpSIP_Msg)
{
	...
	if (strlen(Outbound_PhoneNo)<=0)	return FALSE;
	...
	if ((Invite_Cur=Check_Invite(lpSIP_Msg))>=0)
	{
		if (!SIP_Invite_Data[Invite_Cur].OutBound_Flag)
		{
			...
			{	
				...
				{ SIP_Invite_Data[Invite_Cur].OutBound_Flag=TRUE; Filter_Flag=TRUE;}
				else	Filter_Flag=FALSE;
			}else	Filter_Flag=FALSE;
		}else	Filter_Flag=TRUE;
	}else	Filter_Flag=FALSE;
	...
	{	
		...
		{	
			...
			case 2:	// Response and <--
				...
				{	// 補上總機號碼
					Add_Name(lpSIP_Msg->Cmd_Str,"To:",Outbound_PhoneNo); 
					if (Get_Position(lpSIP_Msg->Cmd_Str,"Contact:")>0)	Add_Name(lpSIP_Msg->Cmd_Str,"Contact:",Outbound_PhoneNo); 
					Transform_Flag=TRUE;
				}
				break;
			case 3: // Response and -->
	...
	if ((Invite_Cur=Check_Invite(lpSIP_Msg))>=0)
	{
		if (!SIP_Invite_Data[Invite_Cur].InBound_Flag)
		{
			...
			{	
				...
				{ SIP_Invite_Data[Invite_Cur].InBound_Flag=TRUE; Filter_Flag=TRUE;}
				else	Filter_Flag=FALSE;
			}else	Filter_Flag=FALSE;
		}else	Filter_Flag=TRUE;
	}else	Filter_Flag=FALSE;
		...
		{	
			...
			case 2:	// Response and <--
				...
				{	// 補上總機號碼
					...
					if (Get_Position(lpSIP_Msg->Cmd_Str,"Contact:")>0)	Edit_Name(lpSIP_Msg->Cmd_Str,"Contact:",lpInvite_Data->Replace_Code);
					Transform_Flag=TRUE;
				}
				break;
			case 3: // Response and -->
...
bool CSIP_Server::ReplaceCode_Filter(SIP_MSG *lpSIP_Msg)
{
	...
	if (!ReplaceCode_Check)	return FALSE;
	...
	if ((Invite_Cur=Check_Invite(lpSIP_Msg))>=0)
	{
		if (!SIP_Invite_Data[Invite_Cur].ReplaceCode_Flag)
		{
			...
			{	
				...
				for (i=0;i<ReplaceCode_Num;i++)	{ if (!strcmp(ReplaceCode_Table[i][0],lpSIP_Msg->To_Name))	break;}
				if (i<ReplaceCode_Num)
				{	// Check To_Name in Table
					SIP_Invite_Data[Invite_Cur].ReplaceCode_Flag=TRUE; Filter_Flag=TRUE;
...
bool CSIP_Server::OutBound_SubServer_Filter(SIP_MSG *lpSIP_Msg,SUBSERVER_DATA *lpSubServer_Data)
{
	bool	Filter_Flag;
		...
		if (!SIP_Invite_Data[Invite_Cur].OutBound_SubServer_Flag)
		{
			...
			{	
				if ( (Mapping_Cur=Check_SubServer_Mapping(lpSIP_Msg->To_Name))>=0 &&
					Mapping_Data[Mapping_Cur].SubServer_Data[SIP_Invite_Data[Invite_Cur].SubServer_Count].Flag )
				{ 
					memcpy(&SIP_Invite_Data[Invite_Cur].Dest_Data,&Mapping_Data[Mapping_Cur].SubServer_Data[SIP_Invite_Data[Invite_Cur].SubServer_Count++],sizeof(SUBSERVER_DATA));
					Filter_Flag=TRUE; SIP_Invite_Data[Invite_Cur].OutBound_SubServer_Flag=TRUE;
				}else	Filter_Flag=FALSE;
			}else	Filter_Flag=FALSE;
		}else	Filter_Flag=TRUE;
	}else	Filter_Flag=FALSE;
	...
	if (Filter_Flag)
	{
		...
		{	// Request and -->
			memcpy(lpSubServer_Data,&SIP_Invite_Data[Invite_Cur].Dest_Data,sizeof(SUBSERVER_DATA));	
		}
		else if (Flow_State==1)
		{	// Request and <--
			lpSubServer_Data->Flag=TRUE;
			lpSubServer_Data->Socket=SIP_Invite_Data[Invite_Cur].FSocket;
			memcpy(&lpSubServer_Data->Addr,&SIP_Invite_Data[Invite_Cur].FAddr,sizeof(sockaddr_in));
			strcpy(lpSubServer_Data->IP,SIP_Invite_Data[Invite_Cur].FR_IP);
			sprintf(lpSubServer_Data->Port,"%d",ntohs(SIP_Invite_Data[Invite_Cur].FAddr.sin_port));
		}
	}
	return Filter_Flag;
}
...
int CSIP_Server::Check_Calling_Party(SIP_MSG *Data)
{
	...
	if (!strcmp(Data->Command,"202") && ((Command_Cur=Check_Command(Data))>=0 && Command_Data[Command_Cur].State==SIP_REFER))
	{
		if (Check_OnLine(Command_Data[Command_Cur].SIP_Msg.ReferTo_Name)<0)
		{
			...
			Start_UDP_Client(&SIP_Data.TSocket,&SIP_Data.TAddr,0,atoi(SIP_Data.ReferTo_Port),SIP_Data.ReferTo_URL,SIP_SERVER_EVENT,m_hWnd);	// 取得Refer_IP&Port的sockaddr_in
	...
	{  
		if ((Online_Cur=Check_OnLine(Data->From_Name,&Command_Data[Command_Cur].SIP_Msg.FAddr))>=0)
		{
			Data->FSocket=SIP_OnLine_Data[Online_Cur].Socket;
			memcpy(&Data->FAddr,&SIP_OnLine_Data[Online_Cur].Addr,sizeof(sockaddr));
...
int CSIP_Server::Check_Called_Party(SIP_MSG *Data)
{
	...
	{	
		...
		GateWay_Data.Socket=NTUT_GW_Socket;
		memcpy(&GateWay_Data.Addr,&NTUT_GW_Addr,sizeof(sockaddr_in));
		strcpy(GateWay_Data.IP,NTUT_GW_IP);	   
		strcpy(GateWay_Data.Port,NTUT_GW_Port);  
		// Set Forwarding_GW_Flag 
		Data->Forwarding_GW_Flag= strcmp(Data->FR_IP,GateWay_Data.IP) && // Condition 1: 若是從GW送過來的封包將走PC to PC
	...
	{
		Data->TSocket=Connect_Data.Socket; 
		memcpy(&Data->TAddr,&Connect_Data.Addr,sizeof(sockaddr_in));
		strcpy(Data->TV_IP,Connect_Data.IP);	   
		strcpy(Data->TR_IP,Connect_Data.IP);
		Save_OnLine(Data,TO); 
	}
	...
	{	
		Data->TSocket=SIP_OnLine_Data[Online_TCur].Socket;
		memcpy(&Data->TAddr,&SIP_OnLine_Data[Online_TCur].Addr,sizeof(sockaddr));
		...
		if (Check_OnLine(Data->From_Name)<0)	Save_OnLine(Data,FROM);
	}

	// 1.5 Re-Save_Online for Recv Bye form Callee
	else if ( (Invite_Cur>=0 && Online_TCur<0 && !strcmp(Data->Command,"BYE")) &&
			  !Addrcmp(&Data->FAddr,&SIP_Invite_Data[Invite_Cur].TAddr) )
	{
		Data->TSocket=SIP_Invite_Data[Invite_Cur].FSocket;
		memcpy(&Data->TAddr,&SIP_Invite_Data[Invite_Cur].FAddr,sizeof(sockaddr));
	...
	// 2. PC to GW
			  ...
			  ( strcmp(GateWay_Data.IP,Local_IP) &&	// Avoid Auto-loop
				strcmp(Data->FR_IP,GateWay_Data.IP)	// In PC to GW, the "From:" Party can't be GW
			  )
			)
	{	
		Data->TSocket=GateWay_Data.Socket; 
		memcpy(&Data->TAddr,&GateWay_Data.Addr,sizeof(sockaddr_in));
		strcpy(Data->TV_IP,GateWay_Data.IP); 
		strcpy(Data->TR_IP,GateWay_Data.IP);
		Save_OnLine(Data,TO); 
	}
	...
	if ((Invite_Cur=Check_Invite(lpSIP_Msg))>=0)
	{
		...
		{	// Recvfrom
			...
			{	// Determine Flow: Caller --> Callee
				if (!Addrcmp(&SIP_Invite_Data[Invite_Cur].FAddr,&lpSIP_Msg->FAddr))	Flow_State=0;
				else // Determine Flow: Callee <-- Caller
				if (!Addrcmp(&SIP_Invite_Data[Invite_Cur].TAddr,&lpSIP_Msg->FAddr))	Flow_State=1;
			}else	// Response
			{	// Determine Flow: Caller <-- Callee
				if (!Addrcmp(&SIP_Invite_Data[Invite_Cur].TAddr,&lpSIP_Msg->TAddr))	Flow_State=2;
				else // Determine Flow: Callee --> Caller
				if (!Addrcmp(&SIP_Invite_Data[Invite_Cur].FAddr,&lpSIP_Msg->TAddr))	Flow_State=3;
			}
		}
		else	
		{	// Sendto
			if (Command_Type)	// Request
			{	// Determine Flow: Caller --> Callee
				if (!Addrcmp(&lpInvite_Data->TAddr,&lpSIP_Msg->Addr))	Flow_State=4;
				else // Determine Flow: Callee <-- Caller
				if (!Addrcmp(&lpInvite_Data->FAddr,&lpSIP_Msg->Addr))	Flow_State=5;
			}else	// Response
			{	// Determine Flow: Caller <-- Callee
				if (!Addrcmp(&lpInvite_Data->FAddr,&lpSIP_Msg->Addr))	Flow_State=6;
				else // Determine Flow: Callee --> Caller
				if (!Addrcmp(&lpInvite_Data->TAddr,&lpSIP_Msg->Addr))	Flow_State=7;
			}
		}
	}
	else	Flow_State=-1;
...
int CSIP_Server::Filter_String(char *S1,char *S2,char *Name,char *URL,char *Port,char *Tag,char *Branch,char *expires,BOOL *Flag)//,int *Name_Locate)
{
...
char Buf[128];
	 ...
	 for (i=0;i<(int)strlen(S2);i++) if ((S2[i]<'0')||(S2[i]>'9')) break;
	 if (i>=(int)strlen(S2)) return 0; // Status No., 

     // 2. Search Beg_Point, Beg=關鍵字之後位址
     k=(int)strlen(S2); if (k>(int)strlen(S1)) return 0;
     for (i=0;i<=(int)strlen(S1)-k;i++) // 需再修正
	 {   if (S1[i]==S2[0]) 
	     {  for (j=0;j<k;j++) if ( (S1[i+j]|0x20) != (S2[j]|0x20) ) break; // 大小寫均考慮
	        if (j>=k) { Beg=i+k; break; }   }   }
	 if (Beg==0) return 0; // 關鍵字未尋獲

	 // 3. Search End_Point, End=關鍵字串句尾
	 End=(int)strlen(S1)-1;
	 for (i=Beg;i<End;i++) if ((S1[i]==0x0a)||(S1[i]==0x0d)) { End=i-1; break; }

     // 4. Search Mid(@), Mid1(:), Mid, Mid2(;), Mid31(<). Mid32(>)
	 if (Beg<End)
	 {  // 4.1 Mid(@)
		for (i=Beg;i<=End;i++) if (S1[i]=='@') { Mid=i; break; }
		// 4.2 Mid1(:)
	    if (Mid>0)
		{  for (i=Mid-1;i>=Beg;i--) if (S1[i]==':') { Mid11=i; break; }
		   for (i=Mid+1;i<=End;i++) if (S1[i]==':') { Mid12=i; break; }	
		}
	    else
		{  for (i=Beg;i<=End;i++)     if (S1[i]==':') { Mid11=i; break; }
		   if (Mid11>0) { for (i=Mid11+1;i<=End;i++) if (S1[i]==':') { Mid12=i; break; } }
		}
		// 4.3 Mid2(;)
		if (Mid12>0) Beg1=Mid12+1; else if (Mid11>0) Beg1=Mid11+1; else Beg1=Beg;
		if ((Mid>0)&&(Mid>Beg1)) Beg1=Mid+1;
		for (i=Beg1;i<=End;i++) if (S1[i]==';') { Mid2=i; break; }
		// 4.4 Mid31(<)
		for (i=Beg;i<=End;i++) if (S1[i]=='<') { Mid31=i; break; }
		// 4.5 Mid32(>)
		if (Mid31>0) for (i=Mid31+1;i<=End;i++) if (S1[i]=='>') { Mid32=i; break; }
		// 4.6 Flag
		if ((Mid31>0)&&(Mid32>0)) Flag[0]=TRUE;
	 }

	 // 5. Get Name
	 if (Mid>0)
	 {  for (i=Mid-1;i>=Beg;i--) if ( (S1[i]==';')||(S1[i]==':')||(S1[i]=='<')||(S1[i]==' ') ) break;
	    if ( (i!=(Beg-1)) && ((Mid-i-1)>0) && ((Mid-i-1)<SIP_NAME_LENGTH) ) 
		{  memcpy(Name,&S1[i+1],Mid-i-1); Name[Mid-i-1]=0; 
	 ...
	 {  Beg1=Mid+1;
	    for (i=Beg1;i<=End;i++) if ( (S1[i]==';')||(S1[i]==':')||(S1[i]=='>')||(S1[i]==' ') ) { End1=i-1; break; } 
	    if (i>End) End1=End;	 }
	 else if ((Mid11>0)&&(Mid11<Mid12))	// with  sip: --> 1.1.1.1 <-- :5060 
	 {  Beg1=Mid11+1; End1=Mid12-1; if (Beg1>End1) { Beg1=End1=0; }  }
	 ...
	 {  if (S1[Mid11-1]=='p') 
		...
		{  End1=Mid11-1;
	       for (i=End1;i>=Beg;i--) if ( (S1[i]==';')||(S1[i]==':')||(S1[i]=='<')||(S1[i]==' ') ) { Beg1=i+1; break; } }
	 }
	 else if (Mid11==0 && Mid12==0 && Mid2>0)	// SIP/2.0/UDP 1.1.1.1;
	 {
		 End1=Mid2-1;
		 for (i=End1;i>=Beg;i--)	if (S1[i]==' ')	{ Beg1=i+1; break;}
		 if (i<Beg)	Beg1=Beg;
	 }
	 else                                // INVITE 1.1.1.1  SIP/2.0
	 {  for (i=Beg;i<=End;i++) if (S1[i]!=' ') { Beg1=i; break; }
	    if (Beg1>0) 
		{  for (i=Beg1;i<=End;i++) if ( (S1[i]==';')||(S1[i]==':')||(S1[i]=='>')||(S1[i]==' ') ) { End1=i-1; break; } 
		   if (i>End) End1=End; }
	 }
	 if ( (End1>0) && (End1>=Beg1) && ((End1-Beg1+1)<SIP_URL_LENGTH) )
	 {  memcpy(URL,&S1[Beg1],End1-Beg1+1); URL[End1-Beg1+1]=0; }
	 ...
	 {  Beg1=Mid11+1;
	    for (i=Beg1;i<=End;i++)  if ( (S1[i]==';')||(S1[i]==':')||(S1[i]=='>')||(S1[i]==' ') ) { End1=i-1; break; } 
		if (i>End) End1=End;
        for (i=Beg1;i<=End1;i++) if ((S1[i]<'0')||(S1[i]>'9')) break;
		if (i<End1) { Beg1=End1=0; }  }

	 if ( (End1>0) && (End1>=Beg1) && ((End1-Beg1+1)<SIP_PORT_LENGTH) )
	 {  memcpy(Port,&S1[Beg1],End1-Beg1+1); Port[End1-Beg1+1]=0; }
	 ...
	 {  for (i=Mid2+1;i<=(End-(int)strlen(Buf)+1);i++) 
	    {  j=0;
	       if ( (S1[i]|0x20) == (Buf[0]|0x20) ) for (j=0;j<(int)strlen(Buf);j++) if ( (S1[i+j]|0x20) != (Buf[j]|0x20) ) break;
		   if (j>=(int)strlen(Buf)) { Beg1=i+(int)strlen(Buf); break; }  }
	    
		if (Beg1>0)
		{  for (i=Beg1;i<=End;i++) if ( (S1[i]==';')||(S1[i]==' ') ) { End1=i-1; break; } 
	       if (i>End) End1=End;
		}
	    if ( (End1>0) && (End1>=Beg1) && ((End1-Beg1+1)<SIP_TAG_LENGTH) )
		{  memcpy(Tag,&S1[Beg1],End1-Beg1+1); Tag[End1-Beg1+1]=0; }
	 }
	 ...
	 {  for (i=Mid2+1;i<=(End-(int)strlen(Buf)+1);i++) 
	    ...
	    if ( (End1>0) && (End1>=Beg1) && ((End1-Beg1+1)<SIP_TAG_LENGTH) )
		{  memcpy(Branch,&S1[Beg1],End1-Beg1+1); Branch[End1-Beg1+1]=0; }
	 }
	 ...
	 {  for (i=Mid2+1;i<=(End-(int)strlen(Buf)+1);i++) 
	    ...
	    if ( (End1>0) && (End1>=Beg1) && ((End1-Beg1+1)<SIP_TAG_LENGTH) )
		{  memcpy(expires,&S1[Beg1],End1-Beg1+1); expires[End1-Beg1+1]=0; }
	 }

	 if ( (strlen(Name)>0) || (strlen(URL)>0) || (strlen(Port)>0) ) return End+1;
	 else                                                           return 0;
}
int  CSIP_Server::Get_Field(char *S1,char *Head,char *Field)
{
int  i,Beg,End;
	 ...
	 {  Beg=i+(int)strlen(Head);
	    for (i=Beg;i<(int)strlen(S1);i++) if  (S1[i]!=' ') { Beg=i; break; }
		for (i=Beg;i<(int)strlen(S1);i++) if ((S1[i]==' ')||(S1[i]==0x0a)||(S1[i]==0x0d)||(S1[i]==';')) { End=i-1; break; }
		if ((Beg!=0)&&(Beg<=End)) strncpy(Field,&S1[Beg],(End-Beg+1)); Field[End-Beg+1]=0;
	    return 0; }
	 else { return -1; }
}

int  CSIP_Server::Get_CSeq_Num(char *S1,char *Head,char *Buf)
{
int  i,j,k,Beg,End;
     ...
     if ( (i=Get_Position(S1,Head)) > 0 )
	 {  Beg=i+(int)strlen(Head);
	    for (j=Beg;j<(int)strlen(S1);j++) if ((S1[j]==0x0a)||(S1[j]==0x0d)) { End=j-1; break; }
	    if (End>Beg)
		{  for (k=End;k>=Beg;k--) if (S1[k]!=0x20) { End=k; break; }
		   for (k=End-1;k>=Beg;k--) if (S1[k]==0x20) { Beg=k+1; break; }
		   if (Beg<End) { strncpy(Buf,&S1[Beg],(End-Beg+1)); Buf[End-Beg+1]=0; return 0; }
		   else return -1;  }
		else return -1;
	 }
	 else return -1;
...
int  CSIP_Server::Modify_CSeq(char *S1,char *Head)
{
int  i,j,k,Beg,End;
char SS1[SIP_MSG_LEGNTH];
     Beg=End=0;
     if ( (i=Get_Position(S1,"CSeq:")) > 0 )
	 {  for (j=i+5;j<(int)strlen(S1);j++) if ((S1[j]==0x0a)||(S1[j]==0x0d)) { End=j-1; break; }
	    for (k=j-1;k>=i+5;k--) if (S1[k]!=0x20) { End=k; break; }
		for (k=End-1;k>=i+5;k--) if (S1[k]==0x20) { Beg=k+1; break; }
		if (Beg<End)
		{  strncpy(SS1,S1,Beg); SS1[Beg]=0;
		   strcat(SS1,Head);
		   strcat(SS1,&S1[End+1]);
...
int  CSIP_Server::Get_Position(char *S1,char *Head)
{
int  i,j;

     for (i=0;i<(int)(strlen(S1)-strlen(Head));i++)
	 {   j=0;
		 if ((S1[i]|0x20)==(Head[0]|0x20)) { for (j=1;j<(int)strlen(Head);j++) if ( (S1[i+j]|0x20) != (Head[j]|0x20) ) break; }
	     if (j>=(int)strlen(Head)) break;  }
...
int  i,j;
	 ...
	 if ( (i=Get_Position(S1,"Content-Length:")) > 0 )
	 {  // 2. Search "v="
		if ( (j=Get_Position(&S1[i],"v=")) > 0 ) strcpy(SDP,&S1[i+j]);	 }
	 if (strlen(SDP)>0) return 0; else return -1;
}

// **************************************************************************************************************
// **************************************************************************************************************
// Process Outbound PhoneNo.
int  CSIP_Server::Add_Name(char *Cmd_Str,char *Head,char *Outbound_PhoneNo)
{
	char	SS1[SIP_MSG_LEGNTH];
	int		i,j,k,Beg,End;
	...
	{	for (j=i;j<(int)strlen(Cmd_Str);j++) if ((Cmd_Str[j]==0x0a)||(Cmd_Str[j]==0x0d)) break;
		for (k=i;k<j;k++) if (Cmd_Str[k]=='@') break;
		if (k<j) 
		{	for (j=k-1;j>=i;j--) if ((Cmd_Str[j]==0x20)||(Cmd_Str[j]==':')||(Cmd_Str[j]=='<')) { Beg=j+1; break; }
			if (Beg>0) 
			{	strncpy(SS1,Cmd_Str,Beg); SS1[Beg]=0;
...
int  CSIP_Server::Delete_Name(char *Cmd_Str,char *Head,char *Outbound_PhoneNo)
{
	char	SS1[SIP_MSG_LEGNTH];
	int		i,j,k,Beg,End;
	...
	{	for (j=i;j<(int)strlen(Cmd_Str);j++) if ((Cmd_Str[j]==0x0a)||(Cmd_Str[j]==0x0d)) break;
		...
		if (k<j) 
		{	for (j=k-1;j>=i;j--) if ((Cmd_Str[j]==0x20)||(Cmd_Str[j]==':')||(Cmd_Str[j]=='<')) { Beg=j+1; break; }
			if (Beg>0) for (j=Beg;j<k;j++) if (Cmd_Str[j]=='-') { End=j; break; }
			if (Beg<=End)
			{	strncpy(SS1,Cmd_Str,Beg); SS1[Beg]=0;
...
int  CSIP_Server::Edit_Name(char *Cmd_Str,char *Head,char *Forwarding_No)
{
char SS1[1500];
int i,j,k,Beg,End;
    ...
    if ( (i=Get_Position(Cmd_Str,Head)) >= 0 )
	{  for (j=i;j<(int)strlen(Cmd_Str);j++) if ((Cmd_Str[j]==0x0a)||(Cmd_Str[j]==0x0d)) break;
	   for (k=i;k<j;k++) if (Cmd_Str[k]=='@') { End=k; break; }
	   if (k<j) 
	   {  for (j=k-1;j>=i;j--) if ((Cmd_Str[j]==0x20)||(Cmd_Str[j]==':')||(Cmd_Str[j]=='<')) { Beg=j+1; break; }
	      if (Beg>0) 
		  {  strncpy(SS1,Cmd_Str,Beg); SS1[Beg]=0;
...
int  CSIP_Server::Get_Auth(char *S1,char *Head,char *Buf)
{
int  i,j,Beg,End;

     Beg=End=Buf[0]=0;
     if ( (i=Get_Position(S1,Head)) > 0 )
	 {  for (j=i+strlen(Head);j<(int)strlen(S1);j++)   if ((S1[j]!=0x20)&&(S1[j]!=0x0a)&&(S1[j]!=0x0d)&&(S1[j]!='"')) { Beg=j; break; }
	    if (Beg>0) for (j=Beg+1;j<(int)strlen(S1);j++) if ((S1[j]==0x20)||(S1[j]==',')||(S1[j]==0x0a)||(S1[j]==0x0d)||(S1[j]=='"')) { End=j-1; break; }
		if ((End>=Beg)&&(Beg>0)) { strncpy(Buf,&S1[Beg],(End-Beg+1)); Buf[End-Beg+1]=0; }
	 }
	 return 0;
}
...
int CSIP_Server::Insertion(char *S1,int Beg,char *Buf)
{
char SS1[SIP_MSG_LEGNTH];
...
int j,Via_Beg[100],Via_Num=0,i=0;
    ...
    while ( ( (j=Get_Position(&S1[i],"Via:")) > 0 ) && (Via_Num<99) ) { i+=j; Via_Beg[Via_Num]=i; i+=strlen("Via:"); Via_Num++; }
    if (Via_Num>1) // 至少兩個Via才處理之
	{  if ( (j=Get_NextLine(&S1[Via_Beg[Via_Num-1]])) > 0 )
	   {  j+=Via_Beg[Via_Num-1];
	      strncpy(ViaBuf,&S1[Via_Beg[1]],j-Via_Beg[1]); ViaBuf[j-Via_Beg[1]]=0;
...
int j,Via_Beg[100],Via_Num=0,i=0;
	...
	{  if ( (j=Get_NextLine(&S1[Via_Beg[Via_Num-1]])) > 0 )
	   {  j+=Via_Beg[Via_Num-1];
	      strncpy(ViaBuf,&S1[Via_Beg[0]],j-Via_Beg[0]); ViaBuf[j-Via_Beg[0]]=0;  } }
	return 0;
}
int CSIP_Server::Add_Via111(char *S1,char *Port,const char *Branch)
{
	...
	if (strcmp(Branch,""))	sprintf(Buf,"Via: SIP/2.0/UDP %s:%s;branch=%s\r\n",Local_IP,LPort,Branch);
	else	sprintf(Buf,"Via: SIP/2.0/UDP %s:%s;branch=z9hG4bK%lu%lu\r\n",Local_IP,LPort,rand(),rand());
...
int CSIP_Server::Init_Invite()
{
	...
	{
		memset(&SIP_Invite_Data[i],0,sizeof(SIP_INVITE_DATA));
		SIP_Invite_Data[i].Flag=FALSE;
		SIP_Invite_Data[i].Answer_State=NoCall;
	}
    return 0;
}
...
int CSIP_Server::Clear_Invite(SIP_MSG *lpSIP_Msg)
{
	...
	{  
		...
		{ 
			...
			SetTimer(TIMERID_INVITE+Invite_Cur,20000,NULL);
		}
		else if (SIP_Invite_Data[Invite_Cur].Answer_State==Temporary)
		{
			KillTimer(TIMERID_INVITE+Invite_Cur);
			// Clear Invite
			memset(&SIP_Invite_Data[Invite_Cur],0,sizeof(SIP_Invite_Data[Invite_Cur]));
			SIP_Invite_Data[Invite_Cur].Flag = FALSE;
			SIP_Invite_Data[Invite_Cur].Answer_State = NoCall;
			SIP_Invite_Num--;
		}
		return 0;	 
	}
...
int  CSIP_Server::Save_Invite(SIP_MSG *lpSIP_Msg)
{
	...
	{	
		for (Invite_Cur=0;Invite_Cur<SIP_INVITE_SIZE;Invite_Cur++)	if (!SIP_Invite_Data[Invite_Cur].Flag)	break;	//找尋空位儲存
		if (Invite_Cur>=SIP_INVITE_SIZE)	return -2;	//表示沒有空位
		Set_Invite(lpSIP_Msg,Invite_Cur,TRUE);
		// Set File name
		SIP_Invite_Data[Invite_Cur].Invite_Time = CTime::GetCurrentTime();
		GetSystemTime(&SIP_Invite_Data[Invite_Cur].Start_TimeDest);
		SIP_Invite_Num++; SIP_Invite_Data[Invite_Cur].Flag=TRUE;
	}
	else	Set_Invite(lpSIP_Msg,Invite_Cur,FALSE);
...
int CSIP_Server::Set_Invite(SIP_MSG *lpSIP_Msg, int Invite_Cur,bool First_Flag)
{
	...
	memcpy(&SIP_Invite_Data[Invite_Cur].FAddr,&lpSIP_Msg->FAddr,sizeof(sockaddr_in));
	memcpy(&SIP_Invite_Data[Invite_Cur].TAddr,&lpSIP_Msg->TAddr,sizeof(sockaddr_in));
	SIP_Invite_Data[Invite_Cur].FSocket=lpSIP_Msg->FSocket;
	SIP_Invite_Data[Invite_Cur].TSocket=lpSIP_Msg->TSocket;
	...
	if ((Online_Cur=Check_OnLine(lpSIP_Msg->From_Name))>=0)	strcpy(SIP_Invite_Data[Invite_Cur].From_User_Name, SIP_OnLine_Data[Online_Cur].Name); 
	if ((Online_Cur=Check_OnLine(lpSIP_Msg->To_Name))>=0)	strcpy(SIP_Invite_Data[Invite_Cur].To_User_Name, SIP_OnLine_Data[Online_Cur].Name); 
	if (First_Flag==TRUE)
	{
		// Set Cancel_Branch
		sprintf(SIP_Invite_Data[Invite_Cur].Cancel_Branch,"z9hG4bK%lu%lu",rand(),rand());
...
int CSIP_Server::Check_Invite(SIP_MSG *lpSIP_Msg)
{
	...
	{   
		...
		{  	
			if ( !strcmp(SIP_Invite_Data[Cur].CallerID_Name,lpSIP_Msg->CallerID_Name) )	break;
			// Calculate i
			i++; if (i>=SIP_Invite_Num) { Cur=SIP_INVITE_SIZE; break; }	 
		}
	 }
	 if (Cur<SIP_INVITE_SIZE) return Cur; else return -1;
}
...
int CSIP_Server::Write_Invite_Log(SIP_INVITE_DATA *lpInvite_Data,ANSWER_STATE State)
{
	...
	if (Invite_Log_Check)
	{
		if (!lpInvite_Data->Flag && lpInvite_Data->Answer_State==Temporary)	return -1;
		File_Name[0]=S2[0]=S3[0]=ElapsedTime[0]=StartTime[0]=EndTime[0]=AnswerState[0]='\0';
		
		if (Log_Path[(j=strlen(Log_Path)-1)]=='\\' || Log_Path[j]=='/')	{ strcpy(File_Name,Log_Path); File_Name[j]=0;}
		sprintf(File_Name, "%s\\%s",File_Name,lpInvite_Data->Invite_Time.Format("%Y-%m-%d")); Create_Directory(File_Name);
		sprintf(File_Name,"%s\\INVITE_Log %s.txt",File_Name,lpInvite_Data->Invite_Time.Format("%Y-%m-%d"));
		...
		{  
			...
			if (Item>=0)	Item++;
			// First Line Text
			fseek(fp,0,SEEK_END); Len=ftell(fp); fseek(fp,0,SEEK_SET);	//get text length
			if(Len==0)	
			{
			...
			if (i>0)	strncat(S2,"                     ",i*sizeof(char));
			// Answer_State
			if (State==Answer)			strcpy(AnswerState,"Answered");
			else if (State==Abandon)	strcpy(AnswerState,"Abandons");
			else if (State==Busy)		strcpy(AnswerState,"Busy");
			else if (State==Forwarding)	strcpy(AnswerState,"Forwarding");
			else if (State==Issue)		strcpy(AnswerState,"Issue");
			else strcpy(AnswerState,"UnKnow");
			i=strlen("Answer_State")-strlen(AnswerState)-2;
			sprintf(S2,"%s%s ",S2,AnswerState);
			if (i>0)	strncat(S2,"            ",i*sizeof(char));
			// Time Process
			if (lpInvite_Data->Start_Time!=0 && lpInvite_Data->End_Time!=0)
			{
				// Elapsed_Time
				Elapsed_Time=lpInvite_Data->End_Time.GetTime()-lpInvite_Data->Start_Time.GetTime();
				if (Elapsed_Time.GetTotalHours()==0)	sprintf(ElapsedTime,"%s",Elapsed_Time.Format("%H:%M:%S"));	
				else	sprintf(ElapsedTime,"%d:%s",Elapsed_Time.GetTotalHours(),Elapsed_Time.Format("%M:%S"));
				// Start_Time & End_Time
				strcpy(StartTime,lpInvite_Data->Start_Time.Format("%H:%M:%S"));
				strcpy(EndTime,lpInvite_Data->End_Time.Format("%H:%M:%S"));
				sprintf(S2,"%s  %s    %s   %s   ",S2,ElapsedTime,StartTime,EndTime);
			}
			else if (lpInvite_Data->Start_Time==0)
			{	strcpy(StartTime,lpInvite_Data->Invite_Time.Format("%H:%M:%S"));
				sprintf(S2,"%s              %s              ",S2,StartTime);
			}
			else sprintf(S2,"%s                                    ",S2);
			// From_Real_IP (From_Virtual_IP)  -->
			sprintf(S3,"%s (%s)",lpInvite_Data->FR_IP,lpInvite_Data->FV_IP);
			i=strlen(" From_Real_IP (From_Virtual_IP)  ")-strlen(S3);
			if (i>0)	strncat(S2,"                                ",i*sizeof(char));
			sprintf(S2,"%s%s-->",S2,S3);
			// To_Real_IP (To_Virtual_IP)
			sprintf(S3,"%s (%s)",lpInvite_Data->TR_IP,lpInvite_Data->TV_IP);
...
int CSIP_Server::Write_Call_Log(char *Head, char *S1, SIP_MSG *lpSIP_Msg, sockaddr_in *lpAddr)
{
	...
	SYSTEMTIME	TimeDest;
	CTimeSpan	Elapsed_Time;
	long	Crt_Hour,Crt_Min,Crt_Sec,Crt_mSec;
	char	Crt_Time[50];
	FILE	*HsfOut1;
...
//	char	S2[SIP_MSG_LEGNTH],S3[SIP_MSG_LEGNTH]
	...
	if (lpSIP_Msg!=NULL) Invite_Cur=Check_Invite(lpSIP_Msg); else return -1;
	Time1 = CTime::GetCurrentTime(); GetSystemTime(&TimeDest);
	// Get SIP_Command
    for (i=0;i<SIP_COMMAND_NO;i++)	{ if (!strncmp(&SIP_COMMAND[i][0],S1,strlen(&SIP_COMMAND[i][0]))) { Cmd=i; break; }}
	...
	// Get Command Line
	if (!strcmp(Head,"send") || !strcmp(Head,"recv"))	sprintf(Command_Line,"%s\r\n",S1);
	else if ((i=Get_NextLine(S1))>0)	{ strncpy(Command_Line,S1,i);  Command_Line[i]=0;}
	...
	if (Invite_Cur>=0)
	{
		...
		//----- Write Call Log -----
		// Open Call_Log file
		if (Log_Path[(k=strlen(Log_Path)-1)]=='\\' || Log_Path[k]=='/')	{ strcpy(File_Name,Log_Path); File_Name[k]=0;}
		sprintf(File_Name, "%s\\%s",File_Name,SIP_Invite_Data[Invite_Cur].Log_File_Name); Create_Directory(File_Name);
		...
		if (0)//Call_Flows_Check)
		{
			if (!strcmp(Head,"sendto") || !strcmp(Head,"recvfrom"))
			{
				...
				if ( (HsfOut1=fopen(Name,"a+"))!=NULL)
				{
					...
					// Get Elapsed time
					Elapsed_Time=((TimeDest.wDay*24+TimeDest.wHour)*60+TimeDest.wMinute)*60+TimeDest.wSecond;
					Elapsed_Time-=((lpInvite_Data->Start_TimeDest.wDay*24+lpInvite_Data->Start_TimeDest.wHour)*60+lpInvite_Data->Start_TimeDest.wMinute)*60+lpInvite_Data->Start_TimeDest.wSecond;
					if (TimeDest.wMilliseconds<lpInvite_Data->Start_TimeDest.wMilliseconds)
					{	Elapsed_Time-=1;
						Crt_mSec=TimeDest.wMilliseconds-lpInvite_Data->Start_TimeDest.wMilliseconds+1000;
					}
					else	Crt_mSec=TimeDest.wMilliseconds-lpInvite_Data->Start_TimeDest.wMilliseconds;
					Crt_Hour=Elapsed_Time.GetTotalHours();
					Crt_Min =Elapsed_Time.GetMinutes();
					Crt_Sec =Elapsed_Time.GetSeconds();
					// Get Elapsed time String
					if (Crt_Hour>0)	sprintf(Crt_Time,"%d:%d:%d.%d%s",Crt_Hour,Crt_Min,Crt_Sec,Crt_mSec,Space_Time);
					else if (Crt_Min>0)	sprintf(Crt_Time,"%d:%d.%d%s",Crt_Min,Crt_Sec,Crt_mSec,Space_Time);
					else	sprintf(Crt_Time,"%d.%d%s",Crt_Sec,Crt_mSec,Space_Time);
					...
					if (Command_Type || (Start_Cur=strstr(S1,lpSIP_Msg->Command))==NULL)	sprintf(Buf,"   %s %s%s",Command,SDP_Flag? "SDP":"",Space_Flow);
					else 
					{	strcpy(Buf1,Start_Cur);
						for (j=0;j<(int)strlen(Buf);j++)	if (Buf1[j]=='\r'||Buf1[j]=='\n')	{ Buf1[j]=0; break;}
					...
					//--- Graph flows ---
					// First Line Text
					fseek(HsfOut1,0,SEEK_END); Len=ftell(HsfOut1); fseek(HsfOut1,0,SEEK_SET);	//get text length
					if(Len==0)	
					{	
						fprintf(HsfOut1,"          Caller                         Server                         Callee\r\n"); 
						sprintf(Buf_IP,"%s:%d%s",lpInvite_Data->FR_IP,ntohs(lpInvite_Data->FAddr.sin_port),Space_Flow); Buf_IP[35]=0;
						sprintf(Buf_IP,"%s%s%s",Buf_IP,Local_IP,Space_Flow); Buf_IP[62]=0;
						sprintf(Buf_IP,"%s%s:%d",Buf_IP,lpInvite_Data->TR_IP,ntohs(lpInvite_Data->TAddr.sin_port));
						fprintf(HsfOut1,"  %s\r\n",Buf_IP);
						if (!(!strcmp(lpSIP_Msg->Command,"BYE") && !Addrcmp(&lpSIP_Msg->FAddr,&lpInvite_Data->TAddr)))
						{	// It indicate Call flows access in the end if Condition True, so don't handle it
							sprintf(Buf_IP,"%s:%s%s",lpSIP_Msg->FV_IP,lpSIP_Msg->Via_Port,Space_Flow); Buf_IP[38]=0;
							if ((Online_Cur=Check_OnLine(lpSIP_Msg->From_Name))>=0)	sprintf(Buf_IP,"%s%d|",Buf_IP,SIP_OnLine_Data[Online_Cur].Server_Port);
							if ((Online_Cur=Check_OnLine(lpSIP_Msg->To_Name))>=0)	{ sprintf(Buf_IP,"%s%d%s",Buf_IP,SIP_OnLine_Data[Online_Cur].Server_Port,Space_Flow); Buf_IP[66]=0;}
							sprintf(Buf_IP,"%s%s",Buf_IP,lpSIP_Msg->TV_IP);
							fprintf(HsfOut1,"  %s\r\n",Buf_IP);
						}
					}	
					
					if (!strcmp(Head,"recvfrom"))		Transmission_Flag=TRUE;
					else if (!strcmp(Head,"sendto"))	Transmission_Flag=FALSE;
...
int CSIP_Server::Get_Response_Code(char *Response_Code, int Code)
{
	...
	{	// Set pSIP_Event_Table for First Time
		...
		{	strncpy(Buf,SIP_Event_Table[i],3); Buf[3]=0;
			lpSIP_Event_Table[atoi(Buf)-100] = SIP_Event_Table[i];
		}
	}
	if (Response_Code==NULL)	return -1;
	// Get Response Code
	if (100<=Code && Code<700)	
	{
		if (lpSIP_Event_Table[Code-100]!=NULL)	strcpy(Response_Code,lpSIP_Event_Table[Code-100]);
		else	sprintf(Response_Code,"%d %s",Code,Status_Msg[(int)(Code/100)]);
	}
	else	Response_Code[0]=0;
	return 0;
}

void CSIP_Server::Send_Relay_Command(char *Command,SIP_MSG *lpSIP_Msg,COMMAND_QUEUE *lpCommand)
{
int  Total=0;//Get_Total(wParam,"0RL",RTP_Data[Cur].SIP_Msg.FV_IP);
char S1[SIP_MSG_LEGNTH];

     if ( (lpSIP_Msg->RL1_Flag) && (strcmp(Command,"0RL1")) ) return;
	 if ( lpCommand->Relay_TCP_Socket==NULL ) return;
     ...
     Hsf_send(lpCommand->Relay_TCP_Socket,S1,strlen(S1),0);
	 if (!strcmp(Command,"0RL1")) { lpSIP_Msg->RL1_Flag=FALSE; Close_Socket(lpCommand->Relay_TCP_Socket,m_hWnd); }
}

bool CSIP_Server::Addrcmp(sockaddr_in *lpAddr1,sockaddr_in *lpAddr2)
{
	char	IP1[100],IP2[100];
	WORD	Port1,Port2;

	strcpy(IP1,inet_ntoa(lpAddr1->sin_addr));
	strcpy(IP2,inet_ntoa(lpAddr2->sin_addr));
	Port1=ntohs(lpAddr1->sin_port);
	Port2=ntohs(lpAddr2->sin_port);

	if (!strcmp(IP1,IP2) && Port1==Port2)	return FALSE;
	else return TRUE;
}
...
int CSIP_Server::Delete_OneLine(char *S1,char *Head)
{
	int i,j;

	if ((i=Get_Position(S1,Head))>0)
	{	
		if ((j=Get_NextLine(&S1[i]))>0)	{ j+=i; Deletion(S1,i,j-1);}
	}
	return 0;
}
...
int CSIP_Server::Check_SubServer_Mapping(char *Name)
{
	...
	if (Mapping_Num>0 && Mapping_Num<=MAPPING_DATA_SIZE)
	{
		...
		{   
			if (Mapping_Data[Mapping_Cur].Flag)
			{  	
				if (!strcmp(Mapping_Data[Mapping_Cur].Name,Name))	break;
				// Calculate i
				i++; if (i>=Mapping_Num) { Mapping_Cur=MAPPING_DATA_SIZE; break; }	 
			}
		}
	}else Mapping_Cur=MAPPING_DATA_SIZE;
	if (Mapping_Cur<MAPPING_DATA_SIZE)	return Mapping_Cur;	else return -1;
}
...
int CSIP_Server::Save_SubServer_Mapping(MAPPING_DATA *lpMapping_Data)
{
	...
	{
		for (Mapping_Cur=0;Mapping_Cur<MAPPING_DATA_SIZE;Mapping_Cur++) { if (!Mapping_Data[Mapping_Cur].Flag) break;}
...
int CSIP_Server::Set_SubServer_Data(SUBSERVER_DATA *lpSubServer_Data,char *SubServer_IP,char *SubServer_Port)
{
	int	i;

	// 直接建立連線
	for (i=0;i<SERVER_PORT_NUM;i++)	if (SIP_Server_Port[i]==atoi(SubServer_Port))	break;
	if (i<SERVER_PORT_NUM)
	{	// 屬於Server_Port
		lpSubServer_Data->Socket=SIP_Server_Socket[i];
		strcpy(lpSubServer_Data->IP,SubServer_IP);
		strcpy(lpSubServer_Data->Port,SubServer_Port);
		// Addr
		lpSubServer_Data->Addr.sin_family=AF_INET;
		lpSubServer_Data->Addr.sin_addr.s_addr=inet_addr(SubServer_IP);
		lpSubServer_Data->Addr.sin_port=htons(atoi(SubServer_Port));
	}
	// 不屬於Server_Port
	else
	{	
		strcpy(lpSubServer_Data->IP,SubServer_IP);
		strcpy(lpSubServer_Data->Port,SubServer_Port);
		Start_UDP_Client(&lpSubServer_Data->Socket,&lpSubServer_Data->Addr,SIP_SERVER_PORT,atoi(SubServer_Port),SubServer_IP,SIP_SERVER_EVENT,m_hWnd);	
	}
	return 0;
}
...
BOOL CSIP_Server::URL_Filter(char *Url)
{
	...
	{	
		// Initial
		Filter_Flag=0;
		for(i=0;i<4;i++)	IP_Value[i]=-1;
		DN_Size=sizeof(DN_Value)/sizeof(DN_Value[0]);
		IP_Size=sizeof(IP_Value)/sizeof(IP_Value[0]);
		memset(DN_Value,0,sizeof(DN_Value));
		// Check URL
		strcpy(URL,Url);
		for (i=0;i<(int)strlen(URL);i++)	if ((URL[i]<'0' || URL[i]>'9') && URL[i]!='.')	break;
		if (i<(int)strlen(Url))
		{	// Domain Name
			for  (DN_Num=j=0;j<(int)strlen(URL);j++)	{ if (URL[j]=='.') DN_Num++;}
			Filter_Flag=(DN_Num>=2)? 2:0;	// Filter_Flag=2 ==> Domain Name OK
		}
		else
		{	// Pure IP
			for  (j=0;(Token=strtok((j==0)? URL:NULL,"."))!=NULL;j++)	{ if (strcmp(Token,"")) IP_Value[j]=atoi(Token); else break;}
			for (j=0;j<IP_Size;j++)	if (IP_Value[j]<0 || IP_Value[j]>255)	break;
...
int CSIP_Server::SIP_Syntax_Filter(SIP_MSG *lpSIP_Msg)
{
	...
	if (!strcmp(lpSIP_Msg->From_Name,"") || !strcmp(lpSIP_Msg->To_Name,""))	Error_Code=400;
...
int CSIP_Server::Domain_Name_Analysis(char *Url,char *IP)
{
	...
	if (URL_Filter(Url))
	{
		...
		if (i<(int)strlen(URL))
		{
			...
			if ((lpHost=gethostbyname(URL))!=NULL)	strcpy(IP,inet_ntoa(*(LPIN_ADDR)(lpHost->h_addr)));
			else	return -1;
		}else strcpy(IP,URL);
	}else	return -1;
...
bool CSIP_Server::Port_Filter(char *Port)
{
	...
	if ((atoi(Port)<=0 || atoi(Port)>=65536) || strcmp(Buf,Port))	Filter_Flag=FALSE;
	else Filter_Flag=TRUE;
...
int CSIP_Server::Trunk_GW_Set(DEST_DATA	*lpTrunk_GW,char *Url)
{
	...
	{
		if ((Token=strtok(Trunk_GW_Url,":"))!=NULL)	strcpy(Trunk_GW_IP,Token);
		if ((Token=strtok(NULL,":"))!=NULL)	strcpy(Trunk_GW_Port,(strcmp(Trunk_GW_Port,""))? Token:"5060"); else strcpy(Trunk_GW_Port,"5060");	
		if (Domain_Name_Analysis(Trunk_GW_IP,Trunk_GW_IP)<0)	DN_Flag=FALSE; else DN_Flag=TRUE;
	}
	// Set Trunk_GW_Data
	if ((URL_Filter(Trunk_GW_IP) && Port_Filter(Trunk_GW_Port)) && DN_Flag)
	{ 	// Build Trunking GW Connection
		lpTrunk_GW->Flag=TRUE;
		strcpy(lpTrunk_GW->IP,Trunk_GW_IP);
		strcpy(lpTrunk_GW->Port,Trunk_GW_Port);
		if (lpTrunk_GW->Socket!=NULL)	{ closesocket(lpTrunk_GW->Socket); lpTrunk_GW->Socket=0;}
		Start_UDP_Client(&lpTrunk_GW->Socket,&lpTrunk_GW->Addr,SIP_SERVER_PORT,atoi(Trunk_GW_Port),Trunk_GW_IP,SIP_SERVER_EVENT,m_hWnd);}
	else	
	{	// Set NTUT_GW to Trunk_GW 
		lpTrunk_GW->Flag=TRUE;
		strcpy(lpTrunk_GW->IP,NTUT_GW_IP);
		strcpy(lpTrunk_GW->Port,NTUT_GW_Port);
		lpTrunk_GW->Socket=NTUT_GW_Socket;
		memcpy(&lpTrunk_GW->Addr,&NTUT_GW_Addr,sizeof(sockaddr_in));
	}
	return 0;
}

int CSIP_Server::Update_Trunk_GW(char *Old_GW_IP,char *Old_GW_Port)
{
	int		i,Online_Cur;
	...
	{   
		...
		{	
			if ( SIP_OnLine_Data[Online_Cur].Trunk_GW.Flag &&
				 (!strcmp(Old_GW_IP,SIP_OnLine_Data[Online_Cur].Trunk_GW.IP) && !strcmp(Old_GW_Port,SIP_OnLine_Data[Online_Cur].Trunk_GW.Port)) )
			{
				SIP_OnLine_Data[Online_Cur].Trunk_GW.Flag=TRUE;
				SIP_OnLine_Data[Online_Cur].Trunk_GW.Socket=NTUT_GW_Socket;
				memcpy(&SIP_OnLine_Data[Online_Cur].Trunk_GW.Addr,&NTUT_GW_Addr,sizeof(sockaddr_in));
				strcpy(SIP_OnLine_Data[Online_Cur].Trunk_GW.IP,NTUT_GW_IP);
				strcpy(SIP_OnLine_Data[Online_Cur].Trunk_GW.Port,NTUT_GW_Port);
			}	
			i++; if (i>=SIP_OnLine_Num) { Online_Cur=SIP_ONLINE_SIZE; break;}
		}
	}
	if (Online_Cur>=SIP_ONLINE_SIZE) return -1; else return 0;	 
}

int CSIP_Server::Edit_URL(char *S1,char *Head,char *URL)
{
	int  i,j,k,Beg,Mid,Mid11,Mid12,Mid2,Mid31,Mid32,End,Beg1,End1;

	// 2. Search Beg_Point, Beg=關鍵字之後位址
	k=(int)strlen(Head); if (k>(int)strlen(S1)) return 0;
	for (i=0;i<=(int)strlen(S1)-k;i++) // 需再修正
	{   if (S1[i]==Head[0]) 
		{  for (j=0;j<k;j++) if ( (S1[i+j]|0x20) != (Head[j]|0x20) ) break; // 大小寫均考慮
			if (j>=k) { Beg=i+k; break; }   }   }
	if (Beg==0) return 0; // 關鍵字未尋獲
	...
	if ( (End1>0) && (End1>=Beg1))	Edition(S1,Beg1,End1,URL);
	
	return 0;
}

int CSIP_Server::Edit_Port(char *S1,char *Head,char *Port)
{
	int  i,j,k,Beg,Mid,Mid11,Mid12,Mid2,Mid31,Mid32,End,Beg1,End1;
	...
	if ( (End1>0) && (End1>=Beg1) )	Edition(S1,Beg1,End1,Port);
...
struct   sockaddr_in  tcpserver;
  ...
  {  WSA_Get_ErrTxt(ErrMsg); return -1; }
...
struct   sockaddr_in  udpserver;
  ...
  {  WSA_Get_ErrTxt(ErrMsg); return -2; }

  // Set Asynchronized Mode
  if ((err=WSAAsyncSelect(*psockfd,Hwnd,EVENT,FD_READ))<0)
  {  WSA_Get_ErrTxt(ErrMsg); return -3;  }
  ...
  {  err=WSAGetLastError();
	 WSA_Get_ErrTxt(ErrMsg); return -4; }
