...
sockaddr_in Addr;
    ...
    i=recv(Socket,Buf,Len,Flag); 	if (i>=0) Buf[i]=0;
	if ( (i<=0)||(Buf[0]!='?') ) 
...
sockaddr_in Addr;
    ...
    Len11=sizeof(sockaddr); getsockname(Socket,(sockaddr *)&Addr,&Len11); Write_Log("send",Buf,&Addr);
    j=Txt_HSF((char *)Buf,Len); i=send(Socket,Buf,j,Flag); return i-1;  
}
